...
@pytest.mark.parametrize(
    ...
    [
        ...
        (-1, 0, pl.Int8, True),
        (1.5, 4, pl.Utf8, True),
        (True, 3, pl.Boolean, True),
        ("123", 1, pl.Int64(), True),
        ("a", 2, pl.Enum(["a", "b"]), True),
        (b"ab12", 2, pl.Binary, True),
        (None, 3, pl.Struct({"a": pl.Int64}), True),
        (D("1.5"), 1, float, True),
        (date(2023, 2, 2), 2, pl.Datetime("ms", "UTC"), True),
        (datetime(2023, 2, 2, tzinfo=timezone.utc), 1, pl.Datetime("us"), True),
        (time(10, 15), 2, pl.Time, True),
        (timedelta(hours=3), 5, pl.Duration("ns"), True),
        ([[1, 2], [3]], 2, pl.Array(pl.Int64, 3), True),
        ((1, 2, 3), 0, pl.List(pl.Int8), True),
        ({"a": 1, "b": "x"}, 1, str, True),
        (np.int64(42), 5, None, True),
        (np.datetime64("2023-02-02T03:04:05", "us"), 5, None, True),
        (pl.col("a").first(), 2, None, False),
        (pl.sum("a"), 1, pl.Int64, False),
        (pl.Series("a", [1, 2, 3]), 3, None, False),
        (np.array([1, 2, 3]), 2, None, False),
        (np.timedelta64(5, "ns"), 3, None, False),
        (42, True, None, False),
        (42, -1, None, False),
        (42, 5, pl.Object, False),
        ({"a": 1}, 3, pl.dtype_of("a"), False),
        (1.5, 1, BaseExtension, False),
