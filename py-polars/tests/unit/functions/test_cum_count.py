...
def test_cum_count_single_arg(reverse: bool, output: list[int]) -> None:
    ...
    expected = pl.Series("a", output, dtype=pl.Int64).to_frame()
...
def test_cum_count_multi_arg() -> None:
    ...
    expected = pl.DataFrame(
        [
            ...
            pl.Series("b", [0, 1, 2], dtype=pl.Int64),
            pl.Series("c", [1, 1, 2], dtype=pl.Int64),
            pl.Series("d", [1, 2, 2], dtype=pl.Int64),
            pl.Series("e", [0, 0, 0], dtype=pl.Int64),
        ]
    )
    assert_frame_equal(result, expected)
...
def test_cum_count_multi_arg_reverse() -> None:
    ...
    expected = pl.DataFrame(
        [
            pl.Series("a", [3, 2, 1], dtype=pl.Int64),
            pl.Series("b", [2, 2, 1], dtype=pl.Int64),
            pl.Series("c", [2, 1, 1], dtype=pl.Int64),
            pl.Series("d", [2, 1, 0], dtype=pl.Int64),
            pl.Series("e", [0, 0, 0], dtype=pl.Int64),
        ]
    )
    assert_frame_equal(result, expected)
...
def test_series_cum_count() -> None:
    ...
    result = s.cum_count()
    expected = pl.Series([1, 2, 2, 3], dtype=pl.Int64)
