...
def test_when_then_parametric(len: int, dtype: pl.DataType, vals: list[Any], broadcast: list[bool]) -> None:
    ...
    for _ in range(10):
        mask = random.choices([False, True, None], k=len)
        if_true = random.choices(vals + [None], k=len)
        if_false = random.choices(vals + [None], k=len)
        
        py_mask, py_true, py_false = [[c[0]] * len if b else c for b, c in zip(broadcast, [mask, if_true, if_false])]
        pl_mask, pl_true, pl_false = [c.first() if b else c for b, c in zip(broadcast, [pl.col.mask, pl.col.if_true, pl.col.if_false])]
