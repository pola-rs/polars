...
@pytest.mark.skipif(sys.platform == "win32", reason="adbc not available on Windows")
def test_write_database_adbc_commit() -> None:
    ...
    result = pl.read_database("SELECT * FROM test_commit", connection=conn)
    assert_frame_equal(result, df)
...
@pytest.mark.skipif(sys.platform == "win32", reason="adbc not available on Windows")
def test_write_database_adbc_no_commit() -> None:
    ...
    result = pl.read_database("SELECT * FROM test_no_commit", connection=conn)
