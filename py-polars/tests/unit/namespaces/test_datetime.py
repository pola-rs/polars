...
@pytest.mark.parametrize(
    ("dt", "expected"),
    [
        (datetime(2022, 3, 15, 3), datetime(2022, 1, 1, 3)),
        (datetime(2022, 3, 15, 3, 2, 1, 123000), datetime(2022, 1, 1, 3, 2, 1, 123000)),
        (datetime(2022, 3, 15), datetime(2022, 1, 1)),
...
@pytest.mark.parametrize("time_unit", ["ms", "us", "ns"])
def test_quarter_start_datetime(
    dt: datetime,
    expected: datetime,
    time_unit: TimeUnit,
    tzinfo: ZoneInfo | None,
...
@pytest.mark.parametrize(
    ("dt", "expected"),
    [
        (date(2022, 3, 15), date(2022, 1, 1)),
        (date(2022, 4, 1), date(2022, 4, 1)),
        (date(2022, 12, 31), date(2022, 10, 1)),
    ],
)
def test_quarter_start_date(dt: date, expected: date) -> None:
    ser = pl.Series([dt])
    result = ser.dt.quarter_start().item()
    assert result == expected
...
def test_year_start_date(dt: date, expected: date) -> None:
    ser = pl.Series([dt])
    result = ser.dt.year_start().item()
    assert result == expected
...
@pytest.mark.parametrize(
    ("dt", "expected"),
    [
        (datetime(2022, 1, 15, 3), datetime(2022, 3, 31, 3)),
        (
            datetime(2022, 3, 15, 3, 2, 1, 123000),
            datetime(2022, 3, 31, 3, 2, 1, 123000),
        ),
        (datetime(2022, 3, 15), datetime(2022, 3, 31)),
        (datetime(2022, 3, 31), datetime(2022, 3, 31)),
        (datetime(2022, 12, 31), datetime(2022, 12, 31)),
    ],
)
@pytest.mark.parametrize(
    ("tzinfo", "time_zone"),
    [
        (None, None),
        (ZoneInfo("Asia/Vladivostok"), "Asia/Vladivostok"),
    ],
)
@pytest.mark.parametrize("time_unit", ["ms", "us", "ns"])
def test_quarter_end_datetime(
    dt: datetime,
    expected: datetime,
    time_unit: TimeUnit,
    tzinfo: ZoneInfo | None,
...
@pytest.mark.parametrize(
    ("dt", "expected"),
    [
        (datetime(2022, 3, 15, 3), datetime(2022, 12, 31, 3)),
        (
            datetime(2022, 3, 15, 3, 2, 1, 123000),
            datetime(2022, 12, 31, 3, 2, 1, 123000),
        ),
        (datetime(2022, 3, 15), datetime(2022, 12, 31)),
        (datetime(2022, 12, 31), datetime(2022, 12, 31)),
    ],
)
@pytest.mark.parametrize(
...
@pytest.mark.parametrize(
    ...
    [
        (date(2022, 3, 15), date(2022, 3, 31)),
        (date(2022, 4, 30), date(2022, 6, 30)),
        (date(2022, 12, 31), date(2022, 12, 31)),
    ],
)
def test_quarter_end_date(dt: date, expected: date) -> None:
    ser = pl.Series([dt])
    result = ser.dt.quarter_end().item()
    assert result == expected
...
@pytest.mark.parametrize(
    ("dt", "expected"),
    [
        (date(2022, 3, 15), date(2022, 12, 31)),
        (date(2022, 3, 31), date(2022, 12, 31)),
        (date(2023, 1, 1), date(2023, 12, 31)),
    ],
)
def test_year_end_date(dt: date, expected: date) -> None:
    ser = pl.Series([dt])
    result = ser.dt.year_end().item()
    assert result == expected


def test_date_start_end_invalid() -> None:
    ...
    with pytest.raises(
        InvalidOperationError,
        match=r"`quarter_start` operation not supported for dtype `time` \(expected: date/datetime\)",
    ):
        ser.dt.quarter_start()
    with pytest.raises(
        InvalidOperationError,
        match=r"`quarter_end` operation not supported for dtype `time` \(expected: date/datetime\)",
    ):
        ser.dt.quarter_end()
    with pytest.raises(
        InvalidOperationError,
        match=r"`year_start` operation not supported for dtype `time` \(expected: date/datetime\)",
    ):
        ser.dt.year_start()
    with pytest.raises(
        InvalidOperationError,
        match=r"`year_end` operation not supported for dtype `time` \(expected: date/datetime\)",
    ):
        ser.dt.year_end()
