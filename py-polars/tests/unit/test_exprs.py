...
@pytest.mark.parametrize(
    ("value", "from_dtype", "to_dtype", "expected_to_succeed"),
    [
        (-1, pl.Int8, pl.UInt8, False),
        (-1, pl.Int16, pl.UInt16, False),
        (-1, pl.Int32, pl.UInt32, False),
        (-1, pl.Int64, pl.UInt64, False),
        (2**7 - 1, pl.UInt8, pl.Int8, True),
        (2**7, pl.UInt8, pl.Int8, False),
        (2**15 - 1, pl.UInt16, pl.Int16, True),
        (2**15, pl.UInt16, pl.Int16, False),
        (2**31 - 1, pl.UInt32, pl.Int32, True),
        (2**31, pl.UInt32, pl.Int32, False),
        (2**63 - 1, pl.UInt64, pl.Int64, True),
...
def test_lit_int_strict_cast(
    ...
    if expected_to_succeed:
        assert s.cast(to_dtype).item() == value
        assert s.to_frame().select(pl.all().cast(to_dtype)).item() == value
    else:
        ...
        with pytest.raises(pl.exceptions.ComputeError):
            # expr cast
            s.to_frame().select(pl.col("a").cast(to_dtype)).item()
        with pytest.raises(pl.exceptions.ComputeError):
            # lit cast
