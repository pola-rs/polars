...
def test_reshape_invalid(shape: tuple[int]) -> None:
    ...
    with pytest.raises(
        pl.ComputeError,
        match=re.escape(f"cannot reshape len 4 into shape {list(shape)}"),
    ):
        s.reshape(shape)


@pytest.mark.parametrize("shape", [(0, 0), (-1, 0), (0, -1), (-1, -1)])
...
@pytest.mark.parametrize("shape", [(0, 1), (1, 0), (1, -1), (-1, 1), (-2, -2)])
