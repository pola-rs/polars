...
def test_str_replace_dynamic_pattern_26789() -> None:
    ...
    # 3-way broadcast: scalar ca with multi-value pat and val
    # e.g. pl.lit("A").str.replace(pl.col("a"), "")
    df_broadcast = pl.DataFrame({"a": ["A", "B"]})
    out = df_broadcast.select(pl.lit("A").str.replace(pl.col("a"), pl.lit("")))
    assert out.to_dict(as_series=False) == {"literal": ["", "A"]}

    out = df_broadcast.select(pl.lit("A").str.replace_all(pl.col("a"), pl.lit("")))
    assert out.to_dict(as_series=False) == {"literal": ["", "A"]}

    # 3-way broadcast: scalar pat with scalar ca and multi-value val
    df_val_broadcast = pl.DataFrame({"val": ["X", "Y"]})
    out = df_val_broadcast.select(
        pl.lit("hello").str.replace(pl.lit("hello"), pl.col("val"))
    )
    assert out.to_dict(as_series=False) == {"literal": ["X", "Y"]}
