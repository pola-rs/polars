...
def test_to_datetime_ambiguous_or_non_existent() -> None:
    with pytest.raises(
        ComputeError,
        match="datetime '2021-11-07T01:00:00' is ambiguous in time zone 'America/Chicago'",
    ):
    ...
    with pytest.raises(
        ComputeError,
        match="datetime '2021-03-28T02:30:00' is non-existent in time zone 'Europe/Warsaw'",
    ):
...
def test_to_datetime_explicit_plus_format_raises() -> None:
    # "%+" isn't a real strtime directive (jiff has no consistent definition
    # for it - see https://github.com/BurntSushi/jiff), so passing it
    # explicitly as a `format` should raise rather than being special-cased.
    with pytest.raises(InvalidOperationError, match="conversion from `str`"):
        pl.Series(["2020-01-01T00:00:00Z"]).str.to_datetime("%+")

    with pytest.raises(InvalidOperationError, match="conversion from `str`"):
        pl.Series(["2020-01-01T00:00:00Z"]).str.to_datetime("%+", exact=False)
