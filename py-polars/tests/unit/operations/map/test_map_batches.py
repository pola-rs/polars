...
def test_map_batches_group() -> None:
    ...
    assert df.group_by("id").agg(
        pl.col("t").map_batches(lambda s: s.sum(), return_dtype="same")
    ...
    assert df.group_by("id").agg(
        pl.col("t").map_batches(
            lambda s: s.sum(), returns_scalar=True, return_dtype="same"
        )
    ).sort("id").to_dict(as_series=False) == {"id": [0, 1], "t": [11, 35]}
...
def test_map_batches_collect_schema_17327() -> None:
    ...
    q = df.group_by("a").agg(pl.col("b").map_batches(lambda s: s, return_dtype="same"))
