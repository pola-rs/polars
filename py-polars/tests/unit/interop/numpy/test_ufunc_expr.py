...
def test_generalized_ufunc_scalar() -> None:
    ...
    indirect = df.select(
        pl.col("values").map_batches(
            custom_sum, returns_scalar=True, return_dtype="same"
        )
    )
    assert_frame_equal(indirect, pl.DataFrame({"values": 15}))
    indirect = df.select(
        pl.col("values").map_batches(
            custom_sum, returns_scalar=False, return_dtype="same"
        )
    )
    assert_frame_equal(indirect, pl.DataFrame({"values": [15]}))
...
def test_grouped_generalized_ufunc() -> None:
    ...
    result = (
        df.group_by("id")
        .agg(pl.col("values").map_batches(gufunc_mean, return_dtype="same"))
        .sort("id")
    )
