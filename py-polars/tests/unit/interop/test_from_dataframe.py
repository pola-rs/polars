...
def test_allow_copy_removed() -> None:
    df = pl.DataFrame({"a": [1, 2], "b": [3.0, 4.0]})
    with pytest.raises(ArgumentRemovedError):
        pl.from_dataframe(df, allow_copy=False)  # type: ignore[call-arg]
...
def test_from_dataframe_empty_string() -> None:
...
def test_from_dataframe_empty_bool() -> None:
...
def test_from_dataframe_empty_categories() -> None:
...
def test_from_dataframe_pyarrow_table() -> None:
    df = pl.DataFrame(
        {
            "a": [1, 2],
            "b": [3.0, 4.0],
...
def test_from_dataframe_pyarrow_recordbatch() -> None:
    a = pa.array([1, 2])
    b = pa.array([3.0, 4.0])

    batch = pa.record_batch([a, b], names=["a", "b"])
    result = pl.from_dataframe(batch)
