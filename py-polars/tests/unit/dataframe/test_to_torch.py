...
def test_to_torch_dict(df: pl.DataFrame) -> None:
    ...
    assert_tensor(td["y"], torch.tensor([True, False, True, False], dtype=torch.bool))
    assert_tensor(td["z"], torch.tensor([1.5, -0.5, 0.0, -2.0], dtype=torch.float32))
...
def test_to_torch_labelled_dataset(df: pl.DataFrame) -> None:
    ds = df.to_torch("dataset", label="x")
