...
def test_pipe_with_schema() -> None:
    ...
    lf = pl.LazyFrame(
        {"a": [1.0, 2.0], "b": ["1.0", "2.5"], "c": [2.0, 3.0], "d": ["3.0", "4.0"]},
        schema={"a": pl.Float64, "b": pl.String, "c": pl.Float32, "d": pl.String},
    )
    result = lf.pipe_with_schema(cast_to_float_if_necessary, ["a", "b", "c"]).collect()
