...
def test_is_null_followed_by_sum() -> None:
    ...
    expected_df = pl.DataFrame({'group': [0, 1, 2], 'val': [1, 1, 0]}, schema_overrides={'val': pl.UInt32})
...
def test_is_not_null_followed_by_sum() -> None:
    ...
    expected_df = pl.DataFrame({'group': [0, 1, 2], 'val': [2, 0, 1]}, schema_overrides={'val': pl.UInt32})
