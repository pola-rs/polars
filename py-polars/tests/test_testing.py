...
def test_strategy_null_probability(
    ...
    assert s.null_count() < df1.null_count().fold(sum)
    assert df1.null_count().fold(sum) < df2.null_count().fold(sum)
    assert df2.null_count().fold(sum) < df3.null_count().fold(sum)
