"""This is a *verbatim* implementation (with updates for Polars)
of the DataFrame Inerchange Protocol API.


This module implements a private `DIPBuffer`, `DIPColumn`, and a
`DIPDataFrame` for the `from_dataframe` public function.

NOTE: undecided on DIP prefix.
...
Note that any reference to Pandas is likely specific to Pandas.
There are inherent diferences between the Polars and Pandas
implementation (i.e. index metadata).
"""
import enum
...
# TODO: delegate
_NotImplementedError = Exception
...
class Column:
    ...
    def get_buffers(
        ...
        Tuple[Buffer, Any], Optional[Tuple[Buffer, Any]], Optional[Tuple[Buffer, Any]]
    ]:
        """
...
# NOTE: undecided on private vs public
class DataFrame:
    """
...
def __from_dataframe(df: "DataFrame") -> pl.DataFrame:
    """*Private* interchange DataFrame constructing function.
    Builds a Polars DataFrame from expected ``Buffer`` and ``Columns``
    implemented as part of the ``__dataframe__`` API.
...
def from_dataframe(df: "DataFrame", allow_copy: bool = False) -> pl.DataFrame:
    ...
    ixdf = df.__dataframe__(allow_copy=allow_copy)

    # convert DataFrame object to Polars DataFrame
    return __from_dataframe(df=ixdf)
