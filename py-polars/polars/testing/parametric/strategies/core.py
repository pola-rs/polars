...
from polars.datatypes import Boolean, DataType, DataTypeClass
...
def series(  # noqa: D417
    ...
    *,
    ...
    if not (0.0 <= null_probability <= 1.0):
        msg = (
            f"`null_probability` should be between 0.0 and 1.0, got {null_probability}"
        )
    ...
    if size == 0:
        ...
        if strategy is None:
            strategy = data(dtype, allow_infinity=allow_infinities)  # type: ignore[arg-type]
    ...
    if 0.0 < null_probability < 1.0:
        ...
        validity = [random.random() > null_probability for _ in range(size)]
        s = F.select(F.when(Series(validity, dtype=Boolean)).then(s)).to_series()
        ...
        data = {
            c.name: draw(
                series(
                    ...
                    dtype=c.dtype,
                    size=size,
                    null_probability=c.null_probability,  # type: ignore[arg-type]
...
class column:
    ...
    column(name='ccy', dtype=None, strategy=sampled_from(['GBP', 'EUR', 'JPY']), null_probability=None, unique=False)
    """  # noqa: W505
