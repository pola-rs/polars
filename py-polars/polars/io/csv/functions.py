...
def read_csv(
    ...
    *,
    ...
    missing_utf8_is_empty_string
        By default a missing value is considered to be `null`; if you would prefer
        missing :class:`String` values to be treated as the empty string, set this
        parameter to `True`.
    ignore_errors
        ...
        `infer_schema_length=0` to read all columns as :class:`String` to check which
        values might cause an issue.
    try_parse_dates
        Try to automatically parse dates. Most ISO8601-like formats can be inferred, as
        well as a handful of others. If this does not succeed, the column remains of
        data type :class:`String`. If `use_pyarrow=True`, dates will always be parsed.
    n_threads
        Number of threads to use in csv parsing.
        Defaults to the number of physical cpu's of your system.
    infer_schema_length
        Maximum number of lines to read to infer schema.
        If schema is inferred wrongly (e.g. as :class:`Int64` instead of
        :class:`Float64`), try to increase the number of lines used to infer the schema
        or override the inferred dtype for those columns with `dtypes`.
        If set to 0, all columns will be read as :class:`String`.
    ...
    raise_if_empty
        When there is no data in the source file, a `NoDataError` will be raised. If
        this parameter is set to `False`, an empty :class:`DataFrame` (with no columns)
        will be returned instead.
    truncate_ragged_lines
        Whether to truncate lines that are longer than the schema.
...
def read_csv_batched(
    ...
    *,
    ...
    null_values
        ...
        - `str`: All values equal to this string will be `null`.
        - `List[str]`: All values equal to any string in this list will be `null`.
        - `Dict[str, str]`: A dictionary that maps column names to `null` value
          strings.
    missing_utf8_is_empty_string
        By default, missing values are set to `null`; if you would prefer missing
        :class:`String` values to be treated as the empty string, set this parameter to
         `True`.
    ignore_errors
        Try to keep reading lines if some lines yield errors.
        First try `infer_schema_length=0` to read all columns as
        :class:`String` to check which values might cause an issue.
    try_parse_dates
        ...
        the column remains of data type :class:`String`.
    n_threads
        Number of threads to use in csv parsing.
        Defaults to the number of physical cpu's of your system.
    infer_schema_length
    ...
    raise_if_empty
        When there is no data in the source file, a `NoDataError` will be raised. If
        this parameter is set to `False`, `None` will be returned from
        `next_batches(n)` instead.
...
def scan_csv(
    ...
    *,
    ...
    raise_if_empty
        When there is no data in the source, a `NoDataError` will be raised. If this
        parameter is set to `False`, an empty :class:`LazyFrame` (with no columns) will
        be returned instead.
    truncate_ragged_lines
        Whether to truncate lines that are longer than the schema.
