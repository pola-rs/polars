...
def concat(
    ...
    *,
    ...
    if how.startswith("align"):
        # Nest the `if` statements to work around https://github.com/python/mypy/issues/21533.
        if not is_sequence_of(elems, pl.DataFrame):
            if not is_sequence_of(elems, pl.LazyFrame):
