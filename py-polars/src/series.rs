...
impl PySeries {
    ...
    pub fn is_in(&self, list_array: &PySeries) -> PyResult<Self> {
        let list_array = list_array.series.list().map_err(PyPolarsEr::from)?;
        let out = self.series.is_in(list_array).map_err(PyPolarsEr::from)?;
