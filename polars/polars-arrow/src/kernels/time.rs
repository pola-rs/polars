...
fn convert_to_naive_local<T1: TimeZone + std::fmt::Debug + std::fmt::Display, T2: TimeZone>(
    ...
    match from_tz.from_local_datetime(&ndt) {
        ...
        LocalResult::Ambiguous(dt_earliest, dt_latest) => match is_earliest {
            ...
            None => Err(ArrowError::InvalidArgumentError(
                format!("datetime '{}' is ambiguous in time zone '{}'. Please use `is_earliest` to tell how it should be localized.", ndt, from_tz)
            ))
        },
        ...
        TimeUnit::Millisecond => {
            let data = try_unary(
                ...
                |value| {
                    ...
                    Ok(convert_to_naive_local(&from_tz, &to_tz, ndt, is_earliest)?
                        .timestamp_millis())
                },
                ArrowDataType::Int64,
            )?;
...
pub fn replace_timezone(
    ...
    Ok(match from.parse::<chrono_tz::Tz>() {
        Ok(from_tz) => match to.parse::<chrono_tz::Tz>() {
            Ok(to_tz) => convert_to_timestamp(from_tz, to_tz, arr, tu, is_earliest)?,
