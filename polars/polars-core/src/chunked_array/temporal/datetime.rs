...
use super::conversion::{
    naive_datetime_to_datetime_ns, naive_datetime_to_datetime_ms
};
use crate::prelude::*;
...
impl DatetimeChunked {
    pub fn as_datetime_iter(
        ...
        self.downcast_iter()
            .map(move|iter| {
                iter.into_iter()
                    .map(move|opt_v| opt_v.copied().map(|v| {
                        func(v)
                    }))
            })
            .flatten()
    ...
    pub fn weekday(&self) -> UInt32Chunked {
        match self.time_unit() {
            TimeUnit::Nanoseconds => self.apply_kernel_cast::<_, UInt32Type>(datetime_to_weekday_ns),
            TimeUnit::Milliseconds => self.apply_kernel_cast::<_, UInt32Type>(datetime_to_weekday_ms),
        }
    ...
    pub fn minute(&self) -> UInt32Chunked {
        match self.time_unit() {
            ...
            TimeUnit::Milliseconds => self.apply_kernel_cast::<_, UInt32Type>(datetime_to_minute_ms),
        }
    }
    ...
    pub fn second(&self) -> UInt32Chunked {
        match self.time_unit() {
            ...
            TimeUnit::Milliseconds => self.apply_kernel_cast::<_, UInt32Type>(datetime_to_second_ms),
        }
    ...
    pub fn nanosecond(&self) -> UInt32Chunked {
        match self.time_unit() {
            TimeUnit::Nanoseconds =>  self.apply_kernel_cast::<_, UInt32Type>(datetime_to_nanosecond_ns),
            TimeUnit::Milliseconds =>  self.apply_kernel_cast::<_, UInt32Type>(datetime_to_nanosecond_ms)
        }
    ...
    pub fn ordinal(&self) -> UInt32Chunked {
        match self.time_unit() {
            TimeUnit::Nanoseconds => self.apply_kernel_cast::<_, UInt32Type>(datetime_to_ordinal_ns),
            TimeUnit::Milliseconds => self.apply_kernel_cast::<_, UInt32Type>(datetime_to_ordinal_ms),
        }
    ...
    pub fn new_from_naive_datetime(name: &str, v: &[NaiveDateTime], tu: TimeUnit) -> Self {
        let func = match tu {
            TimeUnit::Nanoseconds => {
                naive_datetime_to_datetime_ns
            }
            TimeUnit::Milliseconds =>  {
                naive_datetime_to_datetime_ms
        }};
        let vals = v
            .iter()
            .map(func)
