...
fn agg_fixed_size_list_list<
    F: Fn(&FixedSizeListChunked, bool, &mut Vec<i64>, &mut i64, &mut Vec<ArrayRef>) -> bool,
>(
...
impl AggList for FixedSizeListChunked {
    unsafe fn agg_list(&self, groups: &GroupsProxy) -> Series {
        match groups {
            GroupsProxy::Idx(groups) => {
                let func = |ca: &FixedSizeListChunked,
                            mut can_fast_explode: bool,
                            offsets: &mut Vec<i64>,
                            length_so_far: &mut i64,
                            list_values: &mut Vec<ArrayRef>| {
                ...
                agg_fixed_size_list_list(self, groups.len(), func)
            }
            GroupsProxy::Slice { groups, .. } => {
                let func = |ca: &FixedSizeListChunked,
                            mut can_fast_explode: bool,
