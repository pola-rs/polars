...
fn test_lazy_udf() {
    ...
    let new = df
        .lazy()
        .select([col("sepal.width").map(|s| Ok(s * 200.0), get_output::same_type())])
        .collect()
        .unwrap();
    assert_eq!(
...
fn test_lazy_query_2() {
    ...
    let ldf = df
        .lazy()
        .with_column(col("a").map(|s| Ok(s * 2), get_output::same_type()))
...
fn test_lazy_query_4() {
    ...
    let out = base_df
        ...
        .agg([
            ...
            col("cumcases")
                .apply(|s: Series| Ok(&s - &(s.shift(1))), get_output::same_type())
                .alias("diff_cases"),
        ])
