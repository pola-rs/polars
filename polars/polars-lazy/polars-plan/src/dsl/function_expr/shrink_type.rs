...
pub(super) fn shrink(s: Series) -> PolarsResult<Series> {
    if s.dtype().is_numeric() {
        if s.dtype().is_float() {
            ...
            let max = s.max_as_series().get(0).unwrap().extract::<u64>().unwrap_or(0 as u64);
            ...
            let min = s.min_as_series().get(0).unwrap().extract::<i64>().unwrap_or(0 as i64);
            let max = s.max_as_series().get(0).unwrap().extract::<i64>().unwrap_or(0 as i64);
