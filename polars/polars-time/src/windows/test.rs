...
fn test_rolling_lookback() {
    ...
    assert_eq!(dates.len(), groups.len());
    assert_eq!(groups[0], [1, 4]); // bound: 24:00 -> 02:00     time: 24:00
    assert_eq!(groups[1], [2, 4]); // bound: 00:30 -> 02:30     time: 00:30
    assert_eq!(groups[2], [3, 4]); // bound: 01:00 -> 03:00     time: 01:00
    assert_eq!(groups[3], [4, 4]); // bound: 01:30 -> 03:30     time: 01:30
    assert_eq!(groups[4], [5, 4]); // bound: 02:00 -> 04:00     time: 02:00
    assert_eq!(groups[5], [6, 3]); // bound: 02:30 -> 04:30     time: 02:30
    assert_eq!(groups[6], [7, 2]); // bound: 03:00 -> 05:00     time: 03:00
    assert_eq!(groups[7], [8, 1]); // bound: 03:30 -> 05:30     time: 03:30
    assert_eq!(groups[8], [8, 0]); // bound: 04:00 -> 06:00     time: 04:00
