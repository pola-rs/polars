            ...
            let (_, _, groups) = df
                .groupby_rolling(
                    ...
                    &RollingGroupOptions {
                        ...
                        period: "2d".parse().unwrap(),
                        offset: "-2d".parse().unwrap(),
        ...
        let (time_key, mut keys, groups) = df
            .groupby_dynamic(
                ...
                &DynamicGroupOptions {
                    ...
                    every: "1h".parse().unwrap(),
                    period: "1h".parse().unwrap(),
                    offset: "0h".parse().unwrap(),
                    truncate: true,
    ...
    fn test_truncate_offset() -> PolarsResult<()> {
        ...
        let (mut time_key, keys, _groups) = df
            .groupby_dynamic(
                ...
                &DynamicGroupOptions {
                    index_column: "date".into(),
                    every: "6d".parse().unwrap(),
                    period: "6d".parse().unwrap(),
                    offset: "0h".parse().unwrap(),
                    truncate: true,
                    include_boundaries: true,
                    closed_window: ClosedWindow::Both,
