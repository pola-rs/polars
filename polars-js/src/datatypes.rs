  ...
  unsafe fn from_napi_value(env: sys::napi_env, napi_val: sys::napi_value) -> JsResult<Self> {
    ...
    let val = match ty {
      ...
      ValueType::Object => {
        ...
        // )?;
        // if is_arr {
        //   JsAnyValue::List(unsafe { Vec::<JsAnyValue>::from_napi_value(env, napi_val)? })
        // } else {
...
impl ToNapiValue for JsAnyValue {
  unsafe fn to_napi_value(env: sys::napi_env, val: Self) -> Result<sys::napi_value> {
    match val {
      ...
      JsAnyValue::List(ser) => PySeries::to_napi_value(env, ser.into()),
...
impl From<&JsAnyValue> for DataType {
  fn from(av: &JsAnyValue) -> Self {
    match av {
      ...
      JsAnyValue::Boolean(v) => DataType::Boolean,
