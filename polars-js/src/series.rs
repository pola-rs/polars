...
impl PySeries {
  ...
  //
  #[napi(factory)]
  pub fn new_int_8_array(name: String, arr: Int8Array) -> PySeries {
    Series::new(&name, arr).into()
  }
  #[napi(factory)]
  pub fn new_uint8_array(name: String, arr: Uint8Array) -> PySeries {
    Series::new(&name, arr).into()
  }
  #[napi(factory)]
  pub fn new_uint8_clamped_array(name: String, arr: Uint8ClampedArray) -> PySeries {
    Series::new(&name, arr).into()
  }
  #[napi(factory)]
  pub fn new_int16_array(name: String, arr: Int16Array) -> PySeries {
    Series::new(&name, arr).into()
  }
  #[napi(factory)]
  pub fn new_uint16_array(name: String, arr: Uint16Array) -> PySeries {
    Series::new(&name, arr).into()
  }
  #[napi(factory)]
  pub fn new_int32_array(name: String, arr: Int32Array) -> PySeries {
    Series::new(&name, arr).into()
  }
  #[napi(factory)]
  pub fn new_uint32_array(name: String, arr: Uint32Array) -> PySeries {
    Series::new(&name, arr).into()
  }
  #[napi(factory)]
  pub fn new_float32_array(name: String, arr: Float32Array) -> PySeries {
    Series::new(&name, arr).into()
  }
  #[napi(factory)]
  pub fn new_float64_array(name: String, arr: Float64Array) -> PySeries {
    Series::new(&name, arr).into()
  }
  #[napi(factory)]
  pub fn new_bigint64_array(name: String, arr: BigInt64Array) -> PySeries {
    Series::new(&name, arr).into()
  }
  #[napi(factory)]
  pub fn new_biguint64_array(name: String, arr: BigUint64Array) -> PySeries {
    Series::new(&name, arr).into()
  }
  #[napi(factory)]
  pub fn new_opt_str(name: String, val: Wrap<Utf8Chunked>) -> PySeries {
    let mut s = val.0.into_series();
    s.rename(&name);
    PySeries::new(s)
  }
  #[napi(factory)]
  pub fn new_opt_bool(name: String, val: Wrap<BooleanChunked>, _strict: bool) -> PySeries {
    let mut s = val.0.into_series();
    s.rename(&name);
    PySeries::new(s)
  }
  #[napi(factory)]
  pub fn new_opt_i32(name: String, val: Wrap<Int32Chunked>, _strict: bool) -> PySeries {
    let mut s = val.0.into_series();
    s.rename(&name);
    PySeries::new(s)
  }
  #[napi(factory)]
  pub fn new_opt_i64(name: String, val: Wrap<Int64Chunked>, _strict: bool) -> PySeries {
    let mut s = val.0.into_series();
    s.rename(&name);
    PySeries::new(s)
  }
  #[napi(factory)]
  pub fn new_opt_u64(name: String, val: Wrap<UInt64Chunked>, _strict: bool) -> PySeries {
    let mut s = val.0.into_series();
    s.rename(&name);
    PySeries::new(s)
  }
  #[napi(factory)]
  pub fn new_opt_u32(name: String, val: Wrap<UInt32Chunked>, _strict: bool) -> PySeries {
    let mut s = val.0.into_series();
    s.rename(&name);
    PySeries::new(s)
  }
  #[napi(factory)]
  pub fn new_opt_f32(name: String, val: Wrap<Float32Chunked>, _strict: bool) -> PySeries {
    let mut s = val.0.into_series();
    s.rename(&name);
    PySeries::new(s)
  }
  ...
  #[napi(factory)]
  pub fn new_opt_f64(name: String, val: Wrap<Float64Chunked>, _strict: bool) -> PySeries {
    let mut s = val.0.into_series();
    s.rename(&name);
    PySeries::new(s)
  }
  ...
  pub fn new_opt_date(
    ...
    for item in values.into_iter() {
      match item.get_type()? {
        ...
        _ => return Err(JsPolarsErr::Other("Series must be of date type".to_owned()).into()),
      }
    }
  ...
  pub fn new_list(
    name: String,
    values: napi::JsObject,
    dtype: Wrap<DataType>,
  ) -> napi::Result<PySeries> {
    use crate::list_construction::js_arr_to_list;

    let dtype = dtype.0;
    let s = js_arr_to_list(&name, &values, &dtype)?;
    Ok(s.into())
  ...
  #[napi]
  pub fn rechunk(&mut self, in_place: bool) -> Option<PySeries> {
  ...
  #[napi]
  pub fn bitand(&self, other: &PySeries) -> napi::Result<PySeries> {
    let out = self
      .series
      .bitand(&other.series)
  ...
  #[napi]
  pub fn bitor(&self, other: &PySeries) -> napi::Result<PySeries> {
    let out = self
      .series
      .bitor(&other.series)
      .map_err(PyPolarsErr::from)?;
  ...
  #[napi]
  pub fn bitxor(&self, other: &PySeries) -> napi::Result<PySeries> {
    let out = self
      .series
      .bitxor(&other.series)
      .map_err(PyPolarsErr::from)?;
  ...
  #[napi]
  pub fn cumsum(&self, reverse: Option<bool>) -> PySeries {
    let reverse = reverse.unwrap_or(false);
    self.series.cumsum(reverse).into()
  }
  #[napi]
  pub fn cummax(&self, reverse: Option<bool>) -> PySeries {
    let reverse = reverse.unwrap_or(false);
    self.series.cummax(reverse).into()
  }
  #[napi]
  pub fn cummin(&self, reverse: Option<bool>) -> PySeries {
    let reverse = reverse.unwrap_or(false);
    self.series.cummin(reverse).into()
  }
  #[napi]
  pub fn cumprod(&self, reverse: Option<bool>) -> PySeries {
    let reverse = reverse.unwrap_or(false);
  ...
  #[napi]
  pub fn limit(&self, num_elements: f64) -> PySeries {
    self.series.limit(num_elements as usize).into()
  }
  #[napi]
  pub fn slice(&self, offset: i64, length: f64) -> PySeries {
    self.series.slice(offset, length as usize).into()
  }
  #[napi]
  pub fn append(&mut self, other: &PySeries) -> napi::Result<()> {
    self
      .series
      .append(&other.series)
      .map_err(PyPolarsErr::from)?;
  ...
  #[napi]
  pub fn extend(&mut self, other: &PySeries) -> napi::Result<()> {
    self
      .series
      .extend(&other.series)
      .map_err(PyPolarsErr::from)?;
  ...
  #[napi]
  pub fn filter(&self, filter: &PySeries) -> napi::Result<PySeries> {
  ...
  #[napi]
  pub fn add(&self, other: &PySeries) -> PySeries {
    (&self.series + &other.series).into()
  }
  #[napi]
  pub fn sub(&self, other: &PySeries) -> PySeries {
    (&self.series - &other.series).into()
  }
  #[napi]
  pub fn mul(&self, other: &PySeries) -> PySeries {
    (&self.series * &other.series).into()
  }
  #[napi]
  pub fn div(&self, other: &PySeries) -> PySeries {
    (&self.series / &other.series).into()
  }
  #[napi]
  pub fn rem(&self, other: &PySeries) -> PySeries {
    (&self.series % &other.series).into()
  }
  #[napi]
  pub fn head(&self, length: Option<i64>) -> PySeries {
    (self.series.head(length.map(|l| l as usize))).into()
  }
  #[napi]
  pub fn tail(&self, length: Option<i64>) -> PySeries {
    (self.series.tail(length.map(|l| l as usize))).into()
  }
  #[napi]
  pub fn sort(&self, reverse: Option<bool>) -> PySeries {
    let reverse = reverse.unwrap_or(false);
    self.series.sort(reverse).into()
  }
  #[napi]
  pub fn argsort(&self, reverse: bool, nulls_last: bool) -> PySeries {
    self
      .series
      .argsort(SortOptions {
        descending: reverse,
  ...
  #[napi]
  pub fn unique(&self) -> napi::Result<PySeries> {
    let unique = self.series.unique().map_err(PyPolarsErr::from)?;
    Ok(unique.into())
  }
  #[napi]
  pub fn unique_stable(&self) -> napi::Result<PySeries> {
    let unique = self.series.unique_stable().map_err(PyPolarsErr::from)?;
    Ok(unique.into())
  }
  #[napi]
  ...
  #[napi]
  pub fn arg_unique(&self) -> napi::Result<PySeries> {
  ...
  #[napi]
  pub fn take(&self, indices: Vec<u32>) -> napi::Result<PySeries> {
  ...
  #[napi]
  pub fn take_with_series(&self, indices: &PySeries) -> napi::Result<PySeries> {
  ...
  #[napi]
  pub fn is_not_nan(&self) -> napi::Result<PySeries> {
    let ca = self.series.is_not_nan().map_err(PyPolarsErr::from)?;
    Ok(ca.into_series().into())
  }

  #[napi]
  pub fn is_nan(&self) -> napi::Result<PySeries> {
    let ca = self.series.is_nan().map_err(PyPolarsErr::from)?;
    Ok(ca.into_series().into())
  }

  #[napi]
  pub fn is_finite(&self) -> napi::Result<PySeries> {
    let ca = self.series.is_finite().map_err(PyPolarsErr::from)?;
    Ok(ca.into_series().into())
  }

  #[napi]
  pub fn is_infinite(&self) -> napi::Result<PySeries> {
  ...
  #[napi]
  pub fn arg_true(&self) -> napi::Result<PySeries> {
  ...
  #[napi]
  pub fn explode(&self) -> napi::Result<PySeries> {
    let s = self.series.explode().map_err(PyPolarsErr::from)?;
    Ok(s.into())
  }
  #[napi]
  pub fn take_every(&self, n: i64) -> PySeries {
  ...
  #[napi]
  pub fn eq(&self, rhs: &PySeries) -> napi::Result<PySeries> {
    Ok(Self::new(self.series.equal(&rhs.series).into_series()))
  }

  #[napi]
  pub fn neq(&self, rhs: &PySeries) -> napi::Result<PySeries> {
    Ok(Self::new(self.series.not_equal(&rhs.series).into_series()))
  }

  #[napi]
  pub fn gt(&self, rhs: &PySeries) -> napi::Result<PySeries> {
    Ok(Self::new(self.series.gt(&rhs.series).into_series()))
  }

  #[napi]
  pub fn gt_eq(&self, rhs: &PySeries) -> napi::Result<PySeries> {
    Ok(Self::new(self.series.gt_eq(&rhs.series).into_series()))
  }

  #[napi]
  pub fn lt(&self, rhs: &PySeries) -> napi::Result<PySeries> {
    Ok(Self::new(self.series.lt(&rhs.series).into_series()))
  }

  #[napi]
  pub fn lt_eq(&self, rhs: &PySeries) -> napi::Result<PySeries> {
    Ok(Self::new(self.series.lt_eq(&rhs.series).into_series()))
  }

  #[napi]
  pub fn _not(&self) -> napi::Result<PySeries> {
  ...
  #[napi]
  pub fn to_physical(&self) -> PySeries {
  ...
  pub fn to_typed_array(&self) -> TypedArrayBuffer {
    ...
    series.into()
    //   match dt {
    //     DataType::UInt8 => Uint8Array::with_data_copied(series.u8().unwrap().cont_slice()).into_unknown(),
    //     // DataType::UInt16 => series.u16().unwrap().cont_slice(),
    //     // DataType::UInt32 => series.u32().unwrap().cont_slice(),
    //     // DataType::UInt64 => series.u64().unwrap().cont_slice(),
    //     // DataType::Int8 => series.i8().unwrap().cont_slice(),
    //     // DataType::Int16 => series.i16().unwrap().cont_slice(),
    //     // DataType::Int32 => series.i32().unwrap().cont_slice(),
    //     // DataType::Int64 => series.i64().unwrap().cont_slice(),
    //     // DataType::Float32 => series.f32().unwrap().cont_slice(),
    //     DataType::Float64 => Float64Array::with_data_copied(series.f64().unwrap().cont_slice()).into_unknown(),
  ...
  #[napi]
  pub fn drop_nulls(&self) -> PySeries {
    self.series.drop_nulls().into()
  }

  #[napi]
  pub fn fill_null(&self, strategy: Wrap<FillNullStrategy>) -> napi::Result<PySeries> {
    let series = self
      .series
  ...
  #[napi]
  pub fn is_in(&self, other: &PySeries) -> napi::Result<PySeries> {
    let out = self
      .series
      .is_in(&other.series)
      .map_err(PyPolarsErr::from)?;
  ...
  #[napi]
  pub fn clone(&self) -> PySeries {
    PySeries::new(self.series.clone())
  }

  #[napi]
  pub fn shift(&self, periods: i64) -> PySeries {
    let s = self.series.shift(periods);
    PySeries::new(s)
  }
  #[napi]
  pub fn zip_with(&self, mask: &PySeries, other: &PySeries) -> napi::Result<PySeries> {
  ...
  #[napi]
  pub fn str_lengths(&self) -> napi::Result<PySeries> {
  ...
  #[napi]
  pub fn str_contains(&self, pat: String) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
  ...
  #[napi]
  pub fn str_json_path_match(&self, pat: String) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
    let s = ca
      .json_path_match(&pat)
      .map_err(PyPolarsErr::from)?
  ...
  #[napi]
  pub fn str_extract(&self, pat: String, group_index: i64) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
    let s = ca
  ...
  #[napi]
  pub fn str_replace(&self, pat: String, val: String) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
    let s = ca
      .replace(&pat, &val)
      .map_err(PyPolarsErr::from)?
  ...
  #[napi]
  pub fn str_replace_all(&self, pat: String, val: String) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
    let s = ca
      .replace_all(&pat, &val)
      .map_err(PyPolarsErr::from)?
  ...
  #[napi]
  pub fn str_to_uppercase(&self) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
  ...
  #[napi]
  pub fn str_to_lowercase(&self) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
  ...
  #[napi]
  pub fn str_slice(&self, start: i64, length: Option<i64>) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
    let s = ca
  ...
  #[napi]
  pub fn str_hex_encode(&self) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
  ...
  #[napi]
  pub fn str_hex_decode(&self, strict: Option<bool>) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
    let s = ca
      .hex_decode(strict)
      .map_err(PyPolarsErr::from)?
  ...
  #[napi]
  pub fn str_base64_encode(&self) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
  ...
  #[napi]
  pub fn str_base64_decode(&self, strict: Option<bool>) -> napi::Result<PySeries> {
    let ca = self.series.utf8().map_err(PyPolarsErr::from)?;
    let s = ca
  ...
  #[napi]
  pub fn strftime(&self, fmt: String) -> napi::Result<PySeries> {
    let s = self.series.strftime(&fmt).map_err(PyPolarsErr::from)?;
    Ok(s.into())
  }
  #[napi]
  pub fn arr_lengths(&self) -> napi::Result<PySeries> {
    let ca = self.series.list().map_err(PyPolarsErr::from)?;
    let s = ca.lst_lengths().into_series();
    Ok(PySeries::new(s))
  }
  // #[napi]
  // pub fn timestamp(&self, tu: Wrap<TimeUnit>) -> napi::Result<PySeries> {
  //   let ca = self.series.timestamp(tu.0).map_err(PyPolarsErr::from)?;
  //   Ok(ca.into_series().into())
  // }
  #[napi]
  pub fn to_dummies(&self) -> napi::Result<PySeries> {
    todo!()
  ...
  #[napi]
  pub fn get_list(&self, index: i64) -> Option<PySeries> {
  ...
  #[napi]
  pub fn rolling_sum(&self, options: JsRollingOptions) -> napi::Result<PySeries> {
    let s = self
      .series
  ...
  #[napi]
  pub fn rolling_mean(&self, options: JsRollingOptions) -> napi::Result<PySeries> {
    let s = self
      .series
  ...
  #[napi]
  pub fn rolling_median(&self, options: JsRollingOptions) -> napi::Result<PySeries> {
    let s = self
      .series
  ...
  #[napi]
  pub fn rolling_max(&self, options: JsRollingOptions) -> napi::Result<PySeries> {
    let s = self
      .series
  ...
  #[napi]
  pub fn rolling_min(&self, options: JsRollingOptions) -> napi::Result<PySeries> {
    let s = self
      .series
  ...
  #[napi]
  pub fn rolling_var(&self, options: JsRollingOptions) -> napi::Result<PySeries> {
    let s = self
      .series
  ...
  #[napi]
  pub fn rolling_std(&self, options: JsRollingOptions) -> napi::Result<PySeries> {
    let s = self
      .series
  ...
  #[napi]
  pub fn peak_max(&self) -> PySeries {
    self.series.peak_max().into_series().into()
  }
  #[napi]
  pub fn peak_min(&self) -> PySeries {
  ...
  #[napi]
  pub fn is_first(&self) -> napi::Result<PySeries> {
    let out = self
      .series
      .is_first()
      .map_err(PyPolarsErr::from)?
  ...
  #[napi]
  pub fn round(&self, decimals: u32) -> napi::Result<PySeries> {
    let s = self.series.round(decimals).map_err(PyPolarsErr::from)?;
    Ok(s.into())
  }

  #[napi]
  pub fn floor(&self) -> napi::Result<PySeries> {
    let s = self.series.floor().map_err(PyPolarsErr::from)?;
    Ok(s.into())
  }

  #[napi]
  pub fn ceil(&self) -> napi::Result<PySeries> {
    let s = self.series.ceil().map_err(PyPolarsErr::from)?;
    Ok(s.into())
  }
  #[napi]
  ...
  #[napi]
  pub fn hash(&self, k0: Wrap<u64>, k1: Wrap<u64>, k2: Wrap<u64>, k3: Wrap<u64>) -> PySeries {
    let hb = ahash::RandomState::with_seeds(k0.0, k1.0, k2.0, k3.0);
    self.series.hash(hb).into_series().into()
  }
  #[napi]
  pub fn reinterpret(&self, signed: bool) -> napi::Result<PySeries> {
    let s = reinterpret(&self.series, signed).map_err(PyPolarsErr::from)?;
    Ok(s.into())
  }

  #[napi]
  pub fn mode(&self) -> napi::Result<PySeries> {
    let s = self.series.mode().map_err(PyPolarsErr::from)?;
    Ok(s.into())
  }

  #[napi]
  pub fn interpolate(&self) -> PySeries {
    let s = self.series.interpolate();
    s.into()
  }

  #[napi]
  pub fn rank(&self, method: Wrap<RankMethod>, reverse: Option<bool>) -> napi::Result<PySeries> {
    let reverse = reverse.unwrap_or(false);
  ...
  #[napi]
  pub fn diff(&self, n: i64, null_behavior: Wrap<NullBehavior>) -> napi::Result<PySeries> {
  ...
  #[napi]
  pub fn cast(&self, dtype: Wrap<DataType>, strict: Option<bool>) -> napi::Result<PySeries> {
  ...
  #[napi]
  pub fn abs(&self) -> napi::Result<PySeries> {
    let out = self.series.abs().map_err(PyPolarsErr::from)?;
    Ok(out.into())
  }

  #[napi]
  pub fn reshape(&self, dims: Vec<i64>) -> napi::Result<PySeries> {
    let out = self.series.reshape(&dims).map_err(PyPolarsErr::from)?;
    Ok(out.into())
  }

  #[napi]
  pub fn shuffle(&self, seed: Wrap<u64>) -> PySeries {
    self.series.shuffle(seed.0).into()
  }

  #[napi]
  pub fn extend_constant(&self, value: Wrap<AnyValue>, n: i64) -> napi::Result<PySeries> {
    let out = self
      .series
...
macro_rules! impl_get {
  ($name:ident, $series_variant:ident, $type:ty, $cast:ty) => {
    #[napi]
    pub fn $name(s: &PySeries, index: i64) -> Option<$cast> {
...
#[napi]
pub fn series_get_str(s: &PySeries, index: i64) -> Option<String> {
...
macro_rules! impl_arithmetic {
  ($name:ident, $type:ty, $operand:tt) => {
      #[napi]
      pub fn $name(s: &PySeries, other: JsAnyValue) -> napi::Result<PySeries> {
          let other: $type = other.try_into()?;
          Ok(PySeries::new(&s.series $operand other))
      }
  };
}
...
macro_rules! impl_rhs_arithmetic {
  ($name:ident, $type:ty, $operand:ident) => {
    ...
    pub fn $name(s: &PySeries, other: JsAnyValue) -> napi::Result<PySeries> {
      let other: $type = other.try_into()?;
      Ok(PySeries::new(other.$operand(&s.series)))
    }
  };
}
...
macro_rules! impl_eq_num {
  ($name:ident, $type:ty) => {
    #[napi]
    pub fn $name(s: &PySeries, rhs: JsAnyValue) -> napi::Result<PySeries> {
      let rhs: $type = rhs.try_into()?;
      Ok(PySeries::new(s.series.equal(rhs).into_series()))
    }
  };
}
...
macro_rules! impl_neq_num {
  ($name:ident, $type:ty) => {
    ...
    pub fn $name(s: &PySeries, rhs: JsAnyValue) -> napi::Result<PySeries> {
      let rhs: $type = rhs.try_into()?;
      Ok(PySeries::new(s.series.not_equal(rhs).into_series()))
    }
  };
}
...
macro_rules! impl_gt_num {
  ($name:ident, $type:ty) => {
    ...
    pub fn $name(s: &PySeries, rhs: JsAnyValue) -> napi::Result<PySeries> {
      let rhs: $type = rhs.try_into()?;
      Ok(PySeries::new(s.series.gt(rhs).into_series()))
    }
  };
}
...
macro_rules! impl_gt_eq_num {
  ($name:ident, $type:ty) => {
    ...
    pub fn $name(s: &PySeries, rhs: JsAnyValue) -> napi::Result<PySeries> {
      let rhs: $type = rhs.try_into()?;
      Ok(PySeries::new(s.series.gt_eq(rhs).into_series()))
    }
  };
}
...
macro_rules! impl_lt_num {
  ($name:ident, $type:ty) => {
    ...
    pub fn $name(s: &PySeries, rhs: JsAnyValue) -> napi::Result<PySeries> {
      let rhs: $type = rhs.try_into()?;
      Ok(PySeries::new(s.series.lt(rhs).into_series()))
    }
  };
}
...
macro_rules! impl_lt_eq_num {
  ($name:ident, $type:ty) => {
    ...
    pub fn $name(s: &PySeries, rhs: JsAnyValue) -> napi::Result<PySeries> {
      let rhs: $type = rhs.try_into()?;
      Ok(PySeries::new(s.series.lt_eq(rhs).into_series()))
    }
  };
}
