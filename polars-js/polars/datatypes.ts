...
const POLARS_TYPE_TO_CONSTRUCTOR: Record<string, any> = {
  Float32(name, values, strict?) {
    return pli.PySeries.newOptF64(name, values, strict);
  },
  Float64(name, values, strict?) {
    return pli.PySeries.newOptF64(name, values, strict);
  },
  Int8(name, values, strict?) {
    return pli.PySeries.newOptI32(name, values, strict);
  },
  ...
  Int64(name, values, strict?) {
    return pli.PySeries.newOptI64(name, values, strict);
  },
  UInt8(name, values, strict?) {
    return pli.PySeries.newOptU32(name, values, strict);
  },
  ...
  UInt64(name, values, strict?) {
    return pli.PySeries.newOptU64(name, values, strict);
  },
  ...
  Bool(name, values, strict?) {
    return pli.PySeries.newOptBool(name, values, strict);
  },
  Utf8(name, values, strict?) {
    return (pli.PySeries.newOptStr as any)(name, values, strict);
  },
  Categorical(name, values, strict?) {
    return (pli.PySeries.newOptStr as any)(name, values, strict);
  },
  // Object: pli.PySeries.new_object,
  List(name, values, _strict, dtype) {
    return pli.PySeries.newList(name, values, dtype);
  },
};
