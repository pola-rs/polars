...
export const StringFunctions = (_s: pli.PySeries): StringFunctions => {
  const wrap = (method, ...args): any => {
    const ret = _s[method](...args);

    return new _Series(ret);
  };
  ...
  return {
    ...
    strptime(dtype, fmt?) {
      ...
      return null as any;
      // s
      //   .toFrame().select(col(s.name).str.strptime(dtype, fmt))
      //   .getColumn(s.name);
