...
const col = null as any;
type Expr = any;
const Expr = null as any;

type LazyDataFrame = any;
const LazyDataFrame = null as any;
type GroupBy = any;
const GroupBy = null as any;
const concat = null as any;
...
export interface DataFrame extends Arithmetic<DataFrame>, Sample<DataFrame>, WriteMethods {
   ...
   * ```
   */
  toObject(options: {orient: "col"}): Record<string, any>[]
  toObject(options: {orient: "row"}): Record<string, any>[]
  toObject(options: {orient: "dataframe"}): {name: string, datatype: string, values: any[]}[]
 ...
 * @ignore
 */
export const dfWrapper = (_df: pli.PyDataFrame): DataFrame => {
  const unwrap = (method: keyof pli.PyDataFrame, ...args: any[]) => {
    return _df[method as any](...args);
  };
  const wrap = (method, ...args): DataFrame => {
    return dfWrapper(unwrap(method, ...args));
  };
  ...
  const df = {
    ...
    *[Symbol.iterator]() {
    ...
    getColumn(name) {
      return new _Series(_df.column(name)) as any;
    },
    getColumns() {
      return _df.getColumns().map(s => new _Series(s) as any) as any;
    },
    groupBy(...by) {
      return null as any;
      // GroupBy(_d as any, columnOrColumnsStrict(by))
    },
    hashRows(obj: any = 0n, k1=1n, k2=2n, k3=3n) {
      if (typeof obj === "number" || typeof obj === "bigint") {
        return new _Series(_df.hashRows(BigInt(obj), BigInt(k1), BigInt(k2), BigInt(k3)));
      }
      const o = { k0: obj, k1: k1, k2: k2, k3: k3, ...obj};

      return new _Series(_df.hashRows(
        BigInt(o.k0),
        BigInt(o.k1),
        BigInt(o.k2),
        BigInt(o.k3)
    ...
    hstack(columns) {
      ...
      return wrap("hstack", {
        columns: columns.map(col => col.inner()),
        in_place: false
      });
    },
    insertAtIdx: _df.insertAtIdx as any,
    interpolate() {

      return this.select(pli.col("*").interpolate());
    },
    isDuplicated: () => new _Series(_df.isDuplicated()) as any,
    isEmpty: () => _df.height === 0,
    isUnique: () => new _Series(_df.isUnique()) as any,
    ...
    lazy: () => LazyDataFrame(_df.lazy),
    limit: (length=5) => wrap("head", length),
    max(axis=0) {
      if(axis === 1) {
        return new _Series((_df.hmax() as any)) as any;
      } else {
        return wrap("max");
      }
    },
    mean(axis=0, nullStrategy="ignore") {
      if(axis === 1) {
        return new _Series(_df.hmean(nullStrategy) as any) as any;
      }
    ...
    min(axis=0) {
      if(axis === 1) {
        return new _Series(_df.hmin() as any) as any;
      } else {
        return wrap("min");
      }
    },
    ...
    sort(arg,  reverse=false)  {
      ...
      return wrap("sort", [arg].flat(), reverse);
    ...
    sum(axis=0, nullStrategy="ignore") {
      if(axis === 1) {
        return new _Series(_df.hsum(nullStrategy) as any) as any;
      }
    ...
    toObject(options?) {
      ...
      //   return this.getColumns().reduce((acc, curr: Series) => ({
      //     ...acc,
    ...
    writeParquet(dest?, options = {compression: "uncompressed"}) {
      return _df.writeParquet(dest, options?.compression ?? options) as any;
    ...
    writeAvro(dest?, options = {compression: "uncompressed"}) {
      return null as any;
      // return _df.writeAvro(dest, options?.compression ?? options) as any;
    ...
    writeIPC(dest?, options = {compression: "uncompressed"}) {
      return _df.writeIpc(dest, options?.compression ?? options) as any;
    },
    toSeries: (index) => new _Series(_df.selectAtIdx(index) as any) as any,
    ...
    withColumns(column, ...columns: Expr[] | Series[]) {
      ...
      if(isSeriesArray(columns)) {
        return columns.reduce((acc, curr) => acc.withColumn(curr.inner()), dfWrapper(_df));
      } else {
        return this
          .lazy()
          .withColumns(columns)
