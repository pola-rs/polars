...
impl JsLazyFrame {
  pub fn to_object(env: &napi::Env) -> JsResult<JsObject> {
    ...
    ldf.define_properties(&[
      ...
      // napi::Property::new(env, "new_from_parquet")?.with_method(lazyframe::new_from_parquet),
      // napi::Property::new(env, "new_from_ipc")?.with_method(lazyframe::new_from_ipc),
      ...
      napi::Property::new(env, "optimizationToggle")?.with_method(lazyframe::optimization_toggle),
      ...
      napi::Property::new(env, "withColumnRenamed")?.with_method(lazyframe::with_column_renamed),
