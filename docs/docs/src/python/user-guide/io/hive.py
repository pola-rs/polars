...
for df, part in zip(dfs, parts):
    path = Path("docs/docs-res/data/hive/") / part / "data.parquet"
    Path(path).parent.mkdir(exist_ok=True, parents=True)
    df.write_parquet(path)

    path = Path("docs/docs-res/data/hive_mixed/") / part / "data.parquet"
    Path(path).parent.mkdir(exist_ok=True, parents=True)
    df.write_parquet(path)

# Make sure the file is not empty because path expansion ignores empty files.
Path("docs/docs-res/data/hive_mixed/description.txt").write_text("A")
...
print_paths("docs/docs-res/data/hive/")
# --8<-- [end:init_paths]
...
df = pl.scan_parquet("docs/docs-res/data/hive/").collect()
...
try:
    pl.scan_parquet("docs/docs-res/data/hive_mixed/").collect()
...
df = pl.scan_parquet(
    ...
    "docs/docs-res/data/hive_mixed/**/*.parquet",
...
df = pl.scan_parquet(
    [
        "docs/docs-res/data/hive/year=2024/month=01/data.parquet",
        "docs/docs-res/data/hive/year=2024/month=02/data.parquet",
    ],
).collect()
...
df.write_parquet("docs/docs-res/data/hive_write/", partition_by=["a", "b"])
# --8<-- [end:write_parquet_partitioned]

# --8<-- [start:write_parquet_partitioned_show_paths]
print_paths("docs/docs-res/data/hive_write/")
