...
| Equi semi join        | `join(..., how="semi")`    | Keeps rows from the left that have a match on the right. Discard columns of the right that are not used for matching.                                                                                                                |
| Equi anti join        | `join(..., how="anti")`    | Keeps rows from the left that do not have a match on the right. Discard columns of the right that are not used for matching.                                                                                                         |
