...
You have two options if you wish to use lazy evaluation with a pivot. You may collect the `DataFrame` 
before performing the pivot (sacrificing lazy evaluation benefits), or you may specify the `on_columns` 
parameter upfront to declare the resulting schema statically.
