...
- [Narwhals](https://github.com/narwhals-dev/narwhals) since plotly v6.0.0, and therefore
    running execution natively without conversion overhead.
- the [dataframe interchange protocol](https://data-apis.org/dataframe-api/), which offers zero-copy
    conversion where possible. Note that the protocol does not support all Polars data types
    (e.g. `List`) so your mileage may vary here.
