...
| `PLC_LOG_LEVEL`                     | Controls logging verbosity for the polars-on-premise components (e.g. scheduler/worker).<br>e.g. Info`,`Debug`,`Trace`                                                                                         |
| `POLARS_SKIP_DSL_HASH_VERIFICATION` | Skips internal DSL schema/hash compatibility checks between polars cloud client and polars-on-premise binary.<br>Set to `1` only in controlled environments when working around temporary DSL hash mismatches. |
