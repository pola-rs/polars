A major point of `polars-on-premise` is the system requirements regarding the Python version and
...
The minimal requirement for running `polars-on-premise` is the `polars` package. Each release of
`polars-on-premise` is pinned to a single `polars` release, which can be found in the release
announcement.
...
It's also possible to run `polars-on-premise` using a virtual environment.
