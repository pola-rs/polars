...
| `anonymous_result_dst`                          | string  | Destination for results of queries that do not have an explicit sink. Currently supported local mounted (must be reachable on the exact same path) and S3-based. Both options must be network reachable by scheduler, workers, and client.<br>e.g. `s3://bucket/path/to/key`.<br>e.g. `file:///mnt/storage/polars/results` |
