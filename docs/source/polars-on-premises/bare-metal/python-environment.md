...
!!! info "Version pinning"
    Each release of `polars-on-premises` is pinned to a single `polars` release, which can be found in the release announcement and in `polars-on-premises --version`.
