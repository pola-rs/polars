...
Polars On-Prem is our self-hosting solution to run Polars queries on your own infrastructure
remotely, either executing multiple queries in parallel, or distributing a single query across
multiple nodes for large-scale workloads.

Basic version changelog (including most important released features) and dependencies are listed on
[this page](/polars-on-premises/releases).

## On-Prem Workspace

In this section you can read how to deploy your own cluster using Kubernetes and Helm, and how to
tune to configure to make your deployment ready for production.
...
!!! warning "Egress"
    Self-hosted deployments _require_ access to internet. In case egress is not granted to the
    cluster, it will not be able to authenticate and register, and therefore to start. You can read
    more about this requirement in the [dedicated section](#communication-with-our-services) below.

    If you are interested to run Polars On-Prem in a air-gapped environment instead, see the
    [On-Prem Enterprise](#on-prem-enterprise) section at the bottom of this page.

Start by creating an account at [this page](https://cloud.pola.rs/api/redirects/register) if you do
not already have one, and follow the steps!

#### Name your Workspace

Several options are currently offered when deploying a new cluster. We are here interested in the
Kubernetes one, allowing to deploy a Polars On-Prem cluster on your own infrastructure. The setup
starts with choosing a name for your _workspace_, what we use to identify a cluster.

No worries about typos! A workspace can always be renamed later in its settings.

#### Create a Polars service account

Each cluster -regardless of its deployment type- needs to authenticate and register with our
control plane. This is done via a _service account_, a credential pair (client ID and
client secret) that you create and store on your end. We do not save it on our side, and in case of
loss a new set will need to be generated. Service accounts are managed (created and revoked) in the
...
managed solely within our infrastructure.
...
A few things to unpack in this command, let's get to it:
...
- For remote Polars queries without a specific output sink, Polars On-Prem automatically adds a
  persistent sink. We call this sink the "anonymous results" sink. Infrastructure-wise, this sink
  is backed by S3-compatible storage, which should be accessible from all worker nodes and the
  client. For a lightweight quickstart we opted for
  [SeaweedFS](https://github.com/seaweedfs/seaweedfs), backed by an `emptyDir` (all temporary
  results will be lost upon restart of that service).
...
Once your cluster is deployed and registered with our control plane it should be ready to accept
queries.
...
The cluster is now ready to execute your own Polars queries. The following sections are giving more
...
![Deployed objects](./kubernetes/arch-diag-default.png)
...
- The **scheduler** is available on port 5051, and port-forwarding is required to submit queries
  to the cluster. This is the central service of the cluster, bookkeeping query queue and
  execution.
...
- The **temporary storage** is available on port 8333 (default port for SeaweedFS). Port-forwarding
  allows access to [anonymous results](#anonymous-results), that is, tell the Python client where
  to find the result data.
...
To tune the configuration of each service, report to the documentation of the Helm chart
accessible at [this page](https://github.com/polars-inc/helm-charts/tree/main/charts/polars). A few
...
[Helm chart](https://github.com/polars-inc/helm-charts/tree/main/charts/polars). The three
topics listed below are the main configuration sections to tweak to ready your cluster for
production use.

![Deployed objects](./kubernetes/arch-diag-pvc.png)
...
(_i.e._, MiniIO, DigitalOcean Spaces, _etc._). Support for Azure Blob Storage (ABS) and Google
Cloud Storage (GCS) is underway.
...
By default, `emptyDir` volumes are used on each worker node. You can however decide to use
ephemeral volumes instead for more configuration flexibility; as an alternative, your own S3-
compatible storage can be used.
...
throughput characteristics of your storage backend relative to local volumes. As an example, on
AWS, EBS offers lower latency than S3 but lower throughput. This makes EBS a better fit for
workloads that produce many small shuffle files, while S3 will outperform it when shuffle files are
large.
...
If other workloads run on the cluster however (or if multiple instances of the Polars On-Prem
chart are deployed), pod requests and limits should be allocated to worker nodes via the
[Kubernetes API](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/).
In the same fashion, node selector, taints and tolerations should be used to optimize the topology
of the cluster.
...
If you are interested in deploying one or several clusters without any resource limitations nor
data sharing, on bare-metal machines or in a Kubernetes setup, and in air-gapped environments,
please [sign up here to apply](https://w0lzyfh2w8o.typeform.com/to/zuoDgoMv).
