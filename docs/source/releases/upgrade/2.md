...
| `LazyFrame.profile()`[^1]                                            | 1.43          | none (misleading under the streaming engine)                                                           |
...
| `DataFrame.top_k`/`bottom_k(descending=...)`[^2]                     | 1.0           | `reverse=...`                                                                                          |
| `DataFrame.rolling`/`group_by_dynamic`/`upsample(by=...)`[^2]        | 0.20          | `group_by=...`                                                                                         |
| `DataFrame.join(join_nulls=...)`[^2]                                 | 1.24          | `nulls_equal=...`                                                                                      |
...
`read_csv`/`scan_csv(missing_utf8_is_empty_string=...)`. In practice the two rename cases still get
a hint, just not the typed one: Python's own `TypeError` for an unexpected keyword argument appends
a `Did you mean '...'?` suggestion when a similarly-named parameter exists (`missing_columns`,
`empty_string_is_null`). Pure deletions with no near-name match get no such suggestion.

[^2]: Listed here for `DataFrame` only. The equivalent `LazyFrame` parameter
