...
To keep using the in-memory engine for a SQL query, leave the result lazy with `eager=False` and
pick the engine when you collect it yourself:
...
Since the default `engine="auto"` now resolves to the streaming engine (see above), a bare
`show_graph()` call renders the streaming physical graph rather than the optimized IR graph. The IR
graph is only drawn when `plan_stage="ir"` is passed, or when the engine resolves to the in-memory
engine (`engine="in-memory"`, or via engine affinity).
...
```pycon
>>> lf.group_by("a").agg(pl.all().sum()).show_graph()  # shows the streaming physical plan
```

Use instead, to restore the previous output:

```pycon
>>> lf.group_by("a").agg(pl.all().sum()).show_graph(plan_stage="ir")
```

`explain()` is not affected: it has no `plan_stage` parameter and still prints the optimized IR
plan.
...
The frame-level methods below also respect the height of a zero-width frame now, where they used to
collapse it to `(0, 0)` or raise:

- `reverse()`, `shift()`, `fill_null()`, `interpolate()`, `to_dummies()`, the comparison operators
  (`df == 1`) and `/`, `//` keep the height.
- The frame-level aggregations `count()`, `max()`, `min()`, `sum()`, `mean()`, `median()`, `std()`,
  `var()`, `null_count()`, `quantile()` and `product()` return exactly one row, as they do for any
  other frame (also for a zero-width frame of height `0`).
- `unique()`, `n_unique()`, `is_unique()` and `is_duplicated()` treat every zero-width row as
  identical: `unique()` returns at most one row, `n_unique()` returns `min(height, 1)`,
  `is_duplicated()` is all `True` when the height exceeds one, and `is_unique()` is all `False`.
...
The same applies to the `LazyFrame` counterparts.
...
### Load Arrow, Parquet and Iceberg map columns as the new `Map` dtype

Polars 2.0 introduces a `pl.Map(key, value)` dtype. Map columns coming from Arrow (`pl.from_arrow`,
`pl.Series(pyarrow_array)`, the PyCapsule interface, `pl.Schema(pyarrow_schema)`), Parquet and
Iceberg used to be flattened to `List(Struct({"key": ..., "value": ...}))`. They now load as `Map`,
and their Python values are `dict`s rather than lists of key/value structs.

!!! danger

    This change may silently impact the results of your pipelines.
    No error is raised: the schema of anything read from Arrow or Parquet with map columns
    changes, as does the shape of the values returned by `to_list()`, `to_dicts()` and `rows()`.
    Code that used the `list` or `struct` namespaces on these columns now fails.
...
>>> import pyarrow as pa
>>> tbl = pa.table(
...     {"m": pa.array([[("a", 1), ("b", 2)], None], type=pa.map_(pa.string(), pa.int64()))}
... )
>>> pl.from_arrow(tbl)
shape: (2, 1)
┌────────────────────┐
...
Use instead, to recover the previous shape from a `Map` column:
...
Or, when scanning, to load the column as entries in the first place:

```pycon
>>> pl.scan_parquet(
...     "data.parquet",
...     schema={"m": pl.List(pl.Struct({"key": pl.String, "value": pl.Int64}))},
... ).collect_schema()
Schema({'m': List(Struct({'key': String, 'value': Int64}))})
```

A few related points:

- Inference from Python `dict`s is unchanged: `pl.Series([{"a": 1}])` is still a `Struct`. `Map`
  only arises from an explicit dtype, `list.to_map()`, or Arrow/Parquet/Iceberg/IPC input.
- Keys are unique within a row and never `null`. Duplicate keys in a Parquet map are repaired on
  read: the first position and the last value win.
- `to_arrow()` and `write_parquet()`/`write_ipc()` export a `Map` column as a real Arrow map, and
  round-trip it as `Map`.
- The new `Expr.map`/`Series.map` namespace provides `entries()`, `keys()`, `values()`, `len()`,
  `contains_key()` and `get()`.

### Resolve struct arithmetic supertypes per field

Arithmetic between a `Struct` and a numeric scalar or column used to pick a single dtype for the
whole struct, which could silently keep integer fields integer when combined with a float, and
turned `+` on a `String` field into string concatenation. Each field now resolves its own supertype
with the other operand, and any non-numeric field is rejected.

!!! danger

    This change may silently impact the results of your pipelines.
    Field dtypes and values change without an error for numeric fields; only non-numeric fields
    raise.
...
```pycon
>>> df = pl.DataFrame({"i": [1], "f": pl.Series([2], dtype=pl.Float32), "s": ["a"]})
>>> df.select(pl.struct("i", "f") + 1.5).schema
Schema({'i': Struct({'i': Int64, 'f': Float64})})
>>> df.select(pl.struct("i", "s") + 1)
shape: (1, 1)
┌───────────┐
│ i         │
│ ---       │
...
```pycon
>>> df.select(pl.struct("i", "f") + 1.5).schema
Schema({'i': Struct({'i': Float64, 'f': Float32})})
>>> df.select(pl.struct("i", "s") + 1)
Traceback (most recent call last):
...
polars.exceptions.InvalidOperationError: cannot + a struct with non-numeric field: (field: s, dtype: str)
```

Use instead, to keep non-numeric fields out of the arithmetic:

```pycon
>>> df.select(pl.struct("i", "s").struct.with_fields(pl.field("i") + 1))
```

### Widen the `Decimal` precision of rounding, `sign()` and sums

`round()`, `round_sig_figs()`, `ceil()`, `floor()` and `sign()` on a `Decimal(p, s)` column used to
return `Decimal(p, s)`, even when the result needs one more integer digit: rounding `9.5` gives
`10`, and the sign of `0.5` is `1`. The out-of-range results were stored anyway, and later
operations and exports on them failed or misbehaved. These operations now return
`Decimal(p + 1, s)`, capped at precision 38, where a result that still doesn't fit raises. The
precision only grows when the operation can round up: `round(n)` with `n >= s` and `truncate()` keep
`Decimal(p, s)`, and `sign()` only widens when `p == s`.

`list.sum()` and `cum_sum()` in `group_by().agg()` and `over()` also kept the input precision. They
now return `Decimal(38, s)`, like `sum()` and a plain `cum_sum()`.
...
>>> s = pl.Series([Decimal("9.5"), Decimal("-0.5")], dtype=pl.Decimal(2, 1))
>>> s.round(0)
shape: (2,)
Series: '' [decimal[2,1]]
[
...
To keep the old dtype, cast the result back, which raises for values that don't fit:
`s.round(0).cast(pl.Decimal(2, 1))`.

### Defer SQL query resolution to `collect()`

`pl.sql()`, `SQLContext.execute()` and `LazyFrame.sql()` no longer parse and resolve the query when
called. The query text is stored on the plan and resolved when the plan is optimized, so syntax
errors, unknown tables and unknown columns are raised by `collect()` (or `collect_schema()`) instead
of by the call that built the query. `eager=True` and `DataFrame.sql()` are not affected, since they
collect immediately. As a side effect, `SELECT *` is expanded against the source at collect time,
and plans containing SQL can now be serialized.
...
```pycon
>>> ctx = pl.SQLContext(t=pl.LazyFrame({"a": [1, 2]}))
>>> ctx.execute("SELECT * FROM derived")
Traceback (most recent call last):
...
polars.exceptions.SQLInterfaceError: relation 'derived' was not found
```

After:

```pycon
>>> lf = ctx.execute("SELECT * FROM derived")  # no error yet
>>> lf.collect()
Traceback (most recent call last):
...
polars.exceptions.SQLInterfaceError: relation 'derived' was not found
...
Use instead, if you relied on early validation:

```pycon
>>> ctx.execute("SELECT * FROM derived").collect_schema()
```

### Use SQL result scales for decimal `*` and `/` in SQL

In SQL, multiplying or dividing `Decimal` values used to keep the larger operand scale, like the
expression API. That rounds away most of the result for small values: `0.05 * 0.03` gave `0.00`. SQL
`*` now returns the sum of the operand scales, as the SQL standard requires, and raises if that
exceeds 38. SQL `/` returns Snowflake's scale `max(s1, min(s1 + 6, 12))`, rounded half to even: six
more digits than the dividend, up to 12, and never fewer than the dividend. Both return precision
38, and raise if the result doesn't fit. The expression API is unchanged.
...
>>> from decimal import Decimal
>>> df = pl.DataFrame(
...     {"a": [Decimal("0.05")], "b": [Decimal("0.03")]},
...     schema={"a": pl.Decimal(7, 2), "b": pl.Decimal(7, 2)},
... )
>>> df.sql("SELECT a * b AS product, a / b AS ratio FROM self")
```
...
│ product       ┆ ratio          │
│ ---           ┆ ---            │
│ decimal[38,4] ┆ decimal[38,8]  │
╞═══════════════╪════════════════╡
│ 0.0015        ┆ 1.66666667     │
└───────────────┴────────────────┘
```

To get a smaller scale, cast the result, e.g. `CAST(a / b AS DECIMAL(38, 2))`.

### Type exact SQL numeric literals as `Decimal`, and truncate SQL `%` and `DIV`

SQL now uses exact numeric literals and truncating remainder and integer division:

- A numeric literal without an exponent is exact: `1.5` is `Decimal(2, 1)` instead of `Float64`, so
  multiplying a decimal column by `1.0000` no longer turns it into a float. Literals with an
  exponent (`1.5e0`) stay `Float64`. Combined with a float column, an exact literal takes the
  column's float type, so `f32_col * 1.5` is still `Float32`. A literal with more than 38 digits
  raises; write it with an exponent to get an approximate value.
- `%`/`MOD`, `DIV` and `//` truncate toward zero instead of rounding down, so the remainder takes
  the dividend's sign: `-7 % 2` is `-1`, and `DIV(-7.5, 2)` is `-3`.
- `DIV` of decimals returns an exact `Decimal(38, 0)` instead of `Int64`. `DIV` of integers and
  floats still returns `Int64`, and now raises instead of returning `null` when the quotient doesn't
  fit.
- `ROUND` of a decimal rounds half away from zero: `ROUND(2.5)` is `3.0`. Floats still round half to
  even.

| Query                                       | Before            | After                         |
| ------------------------------------------- | ----------------- | ----------------------------- |
| `SELECT 1.5`                                | `1.5` (`Float64`) | `1.5` (`Decimal(2, 1)`)       |
| `SELECT CAST(1 AS DECIMAL(10, 2)) * 1.0000` | `1.0` (`Float64`) | `1.000000` (`Decimal(38, 6)`) |
| `SELECT -7 % 2`                             | `1`               | `-1`                          |
| `SELECT DIV(-7.5, 2)`                       | `-4` (`Int64`)    | `-3` (`Decimal(38, 0)`)       |
| `SELECT ROUND(2.5)`                         | `2.0` (`Float64`) | `3.0` (`Decimal(3, 1)`)       |

To keep float semantics, write the literal with an exponent (`2.5e0`) or cast it
(`CAST(2.5 AS DOUBLE)`).

### Read the Parquet `ENUM` logical type as `String`

Parquet columns annotated with the `ENUM` logical type (or the legacy `ENUM` converted type) were
read as `Binary`. Per the Parquet specification `ENUM` annotates UTF-8 byte arrays, so they are now
read as `String`. Polars never writes `ENUM` itself; it appears in files produced by writers that
convert from Avro, Thrift or Protobuf. Invalid UTF-8 in such a column now raises a `ComputeError`
instead of being handed back as bytes.
...
```pycon
>>> pl.scan_parquet("enum.parquet").collect_schema()
Schema({'color': Binary})
>>> pl.read_parquet("enum.parquet")["color"][0]
b'RED'
```
...
Use instead, if you need the bytes:

```pycon
>>> pl.read_parquet("enum.parquet").with_columns(pl.col("color").cast(pl.Binary))
```
...
The same applies to casting `String` to `Datetime` and `Time` — use `str.to_datetime()` and
`str.to_time()` instead.

SQL is not affected: `CAST(s AS DATE)`, `s::time`, `TRY_CAST(...)` and friends keep working, since
they now go through the format-inferring `str.to_date()`/`str.to_datetime()`/`str.to_time()` instead
of the removed cast. The error raised for an unparsable string does change, though:

```pycon
>>> pl.DataFrame({"values": ["a", "b"]}).sql("SELECT values::time FROM self")
# before: polars.exceptions.InvalidOperationError: conversion from `str` to `time` failed in column 'values' for 2 out of 2 values: ["a", "b"]
# after:  polars.exceptions.ComputeError: could not find an appropriate format to parse times, please define a format
```
...
where a list is expected, implicitly imploding it. That coercion has warned since it was deprecated
(see [#22149](https://github.com/pola-rs/polars/issues/22149)) and existed only as a pre-2.0
compatibility shim; it has now been removed.
...
Use an explicit `implode` instead; it reproduces the old behavior exactly, which is what the error
message suggests:
...
Note that the flat-column form never indexed per row: `list.gather(pl.col("b"))` imploded all of `b`
into a single index list, so every row was gathered with the same indices. If per-row indexing is
what you want, use `list.get`, or `concat_list` to keep a list-typed result:
...
### Require matching time units in `is_in()` and `list.contains()`

`is_in()` and `list.contains()` used to accept a `Datetime`/`Duration` needle whose time unit was
finer than that of the values being searched, and compared the underlying integers as-is. Any
mismatch in time unit now raises.

!!! danger

    The old behavior returned wrong answers rather than erroring: a nanosecond needle was compared
    against microsecond values without conversion, so matches were missed silently.
...
>>> needle = pl.Series([datetime(2020, 1, 1)], dtype=pl.Datetime("ns"))
>>> values = pl.Series([datetime(2020, 1, 1)], dtype=pl.Datetime("us"))
>>> needle.is_in(values)
shape: (1,)
Series: '' [bool]
[
...
Traceback (most recent call last):
...
polars.exceptions.InvalidOperationError: 'is_in' cannot check for ns values in μs data
...
```pycon
>>> needle.dt.cast_time_unit("us").is_in(values)
shape: (1,)
Series: '' [bool]
[
        true
...
### Stop comparing `Decimal` needles against float data in `is_in()`

`is_in()`, `list.contains()` and `arr.contains()` used to accept a `Decimal` needle searched in
float or integer data by casting the data to the needle's `Decimal` type. That cast rounds floats
onto the needle's scale and raises on integers too large for its precision, so the answer depended
on values unrelated to the needle. A `Decimal` needle in float data now raises. Integer data, and
`Decimal` data of any precision and scale, are compared exactly, without casting to the needle's
type.

!!! danger

    The old behavior returned wrong answers rather than erroring: `1.005` was rounded to `1.00`,
    so it matched a `Decimal("1.00")` needle.
...
```pycon
>>> from decimal import Decimal
>>> prices = pl.Series("price", [Decimal("1.00"), Decimal("2.50")], dtype=pl.Decimal(10, 2))
>>> prices.is_in([1.005, 2.5])
shape: (2,)
Series: 'price' [bool]
[
        true
...
Traceback (most recent call last):
...
polars.exceptions.InvalidOperationError: 'is_in' cannot check for Decimal(10, 2) values in List(Float64) data
```

Use instead, comparing both sides as the same type. Compare as floats:

```pycon
>>> prices.cast(pl.Float64).is_in([1.005, 2.5])
shape: (2,)
Series: 'price' [bool]
[
        false
...
or compare as decimals, by writing the values as decimals:

```pycon
>>> prices.is_in([Decimal("1.005"), Decimal("2.5")])
shape: (2,)
Series: 'price' [bool]
[
        false
...
Both agree that `1.00` is not `1.005`.

### Raise on non-numeric input to `log()`, `log1p()` and `exp()`

These used to return a `Float64` column of `null`s for non-numeric input (or crash on nested
dtypes). They now raise, also from `collect_schema()`. Cast to a numeric dtype first.

```pycon
>>> pl.Series(["a", "b"]).log()
Traceback (most recent call last):
...
polars.exceptions.InvalidOperationError: `log` operation not supported for dtype `str` as argument 0
```
...
| `Config.set_auto_structify()`, `POLARS_AUTO_STRUCTIFY` environment variable[^3]                | 1.32          | explicit `pl.struct(...)`                                                                              |
| `POLARS_STREAMING_CHUNK_SIZE` environment variable[^3]                                         | 1.x           | `POLARS_IDEAL_MORSEL_SIZE`, or `Config.set_streaming_chunk_size()`                                     |
...
| `polars.type_aliases` module[^1]                                                               | 1.0           | `polars._typing`                                                                                       |
...
[^3]: These environment variables are silently ignored rather than raising.
`POLARS_AUTO_STRUCTIFY=1` kept working for a while after `Config.set_auto_structify()` was removed;
multi-output expressions in `select`/`with_columns` are now never wrapped in a struct automatically,
so wrap them yourself with `pl.struct(...)`. `POLARS_STREAMING_CHUNK_SIZE` was a legacy alias of
`POLARS_IDEAL_MORSEL_SIZE`; `Config.set_streaming_chunk_size()` still works and now sets the latter.
...
`Expr`/`Series.cut()` and `Expr`/`Series.qcut()` are deprecated in favour of `bin_intervals()`,
`bin_quantiles()` and `bin_ranks()`. The new methods return an `Enum` of the labels you pass, or the
integer bin index if you pass `labels=False`.
...
`bin_ranks()` is a third option with no `cut`/`qcut` equivalent: it bins on position in sorted
order, so the bins are near-equal-sized even when values repeat, whereas `qcut()` and
`bin_quantiles()` keep equal values together and may produce empty (or imbalanced) bins.

Note the differences when translating:

- The `bin_*` methods are left-closed by default. Pass `right_closed=True` to keep the right-closed
  bins of `cut()`/`qcut()`. (`bin_ranks()` is positional, so it has no `right_closed`.)
- `labels` is required. There is currently no equivalent of the automatic interval-string labels
  (`"(-inf, -1]"` etc.) that `cut()`/`qcut()` generate by default.
- `include_breaks=True` becomes `include_intervals=True`, which returns a struct with `bin`, `left`
  and `right` fields rather than `breakpoint` and `category`.
- `qcut()`'s `allow_duplicates` has no counterpart: `bin_quantiles()` accepts duplicate quantiles
  and simply produces an empty bin between them.
