...
### Keep `Decimal` when combining a `Decimal` column with a float literal

Arithmetic between a `Decimal` column and a Python float literal used to cast the column to
`Float64`. The literal is now converted to `Decimal` instead, with a scale inferred from the
literal, and the result stays `Decimal`. `fill_null()` and `when/then/otherwise` with a float
literal follow the same rule. Combining a `Decimal` column with a `Float64` _column_ still produces
`Float64`.

!!! danger

    This change may silently impact the results of your pipelines.
    No error is raised: the output dtype changes, and values are rounded to the scale of the
    result, so `2.25 * 1.5` is `3.38` rather than `3.375`.
...
>>> df = pl.DataFrame({"d": [Decimal("1.50"), Decimal("2.25")]}, schema={"d": pl.Decimal(10, 2)})
>>> df.select(pl.col("d") * 1.5)
shape: (2, 1)
┌───────┐
│ d     │
│ ---   │
...
│ 3.38          │
└───────────────┘
>>> df.select(pl.col("d") * 1.123456).schema  # the literal's scale widens the result
Schema({'d': Decimal(precision=38, scale=6)})
>>> df.select(pl.col("d").fill_null(0.5)).schema
Schema({'d': Decimal(precision=10, scale=2)})
```

Use instead, to keep computing in floating point:

```pycon
>>> df.select(pl.col("d").cast(pl.Float64) * 1.5)
shape: (2, 1)
┌───────┐
│ d     │
│ ---   │
...
Comparisons against float literals (`pl.col("d") > 1.5`) are not affected; they return the same
booleans as before.
...
Use instead, casting whichever side should decide the comparison. Compare as floats, where `1.00` is
not `1.005`:
...
or compare as decimals, explicitly rounding the floats onto the column's scale:

```pycon
>>> prices.is_in(pl.Series([1.005, 2.5]).cast(pl.Decimal(10, 2)).implode())
shape: (2,)
Series: 'price' [bool]
[
        true
