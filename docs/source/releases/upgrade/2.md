...
```pycon
>>> from decimal import Decimal
>>> values = pl.Series([[1.005]])
>>> values.list.contains(pl.lit(Decimal("1.00"), pl.Decimal(10, 2)))
shape: (1,)
Series: '' [bool]
[
        true
...
Traceback (most recent call last):
...
polars.exceptions.InvalidOperationError: 'list.contains' cannot check for Decimal(10, 2) values in List(Float64) data
```

Use instead, choosing the side to cast explicitly:

```pycon
>>> values.list.contains(pl.lit(Decimal("1.00"), pl.Decimal(10, 2)).cast(pl.Float64))
shape: (1,)
Series: '' [bool]
[
        false
