...
It gains `scan_csv`'s
`with_column_names`, `infer_schema_files`, `credential_provider`, `include_file_paths`,
`extra_columns`, and `missing_columns` parameters, and accepts a list of sources. 

It loses `n_threads`, `batch_size`, `sample_size`, and `rechunk`, which have no equivalent in the lazy reader
(call `.rechunk()` on the result if you need it). Its behavior now also matches `scan_csv` exactly:
a `schema_overrides` list must cover every column in the file, and `columns=[...]` returns columns
in the requested order rather than sorted.
