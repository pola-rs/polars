...
- A numeric literal without an exponent is exact: `1.5` is `Decimal(2, 1)` instead of `Float64`, so
  `0.1 + 0.2` is exactly `0.3`. Literals with an exponent (`1.5e0`) stay `Float64`. Combined with a
  float column, an exact literal takes the column's float type, so `f32_col * 1.5` is still
  `Float32`. A literal with more than 38 digits raises; write it with an exponent to get an
  approximate value.
...
| `SELECT 0.1 + 0.2`    | `0.30000000000000004` (`Float64`) | `0.3` (`Decimal(1, 1)`) |
| `SELECT -7 % 2`       | `1`                               | `-1`                    |
