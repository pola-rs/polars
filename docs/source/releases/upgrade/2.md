...
- `get()` and `contains_key()` cast the key they look up to the Map's key type, never the Map's
  keys. A key that the cast cannot represent exactly, such as `2**40` for `Int32` keys or `1.1` for
  `Float32` keys, is absent. A time-zone-aware key is compared by instant with keys in another zone;
  a naive key cannot be looked up among aware keys, or the other way round.
...
### Convert time units in `is_in()` and `list.contains()`

`is_in()` and `list.contains()` used to accept a `Datetime`/`Duration` needle whose time unit was
finer than that of the values being searched, and compared the underlying integers as-is. The needle
is now converted to the unit of the values. A needle that the conversion cannot represent exactly,
because it is out of range or would be rounded, matches nothing.
...
>>> # 1 ns past midnight has no exact microsecond value, so it is not found.
>>> needle.dt.offset_by("1ns").is_in(values)
shape: (1,)
Series: '' [bool]
[
        false
...
### Stop comparing `Decimal` needles against float and integer data in `is_in()`
...
on values unrelated to the needle. This combination now raises, as `Decimal` against any other
non-`Decimal` type already did.
...
### Raise when comparing naive and time-zone-aware datetimes in `is_in()`

`is_in()`, `list.contains()` and `arr.contains()` used to compare a naive `Datetime` against
time-zone-aware values, or the other way round, by their underlying integers, reading the naive side
as UTC. This now raises, as `==` already did. Two time-zone-aware sides are still compared by
instant, whatever their zones.
...
```pycon
>>> from datetime import datetime
>>> values = pl.Series([[datetime(2020, 1, 1)]]).list.eval(pl.element().dt.replace_time_zone("UTC"))
>>> values.list.contains(pl.lit(datetime(2020, 1, 1)))
shape: (1,)
Series: '' [bool]
[
        true
...
Traceback (most recent call last):
...
polars.exceptions.InvalidOperationError: 'list.contains' cannot check for Datetime('μs') values in List(Datetime('μs', 'UTC')) data, as one is time-zone-aware and the other is not
...
```pycon
>>> values.list.contains(pl.lit(datetime(2020, 1, 1)).dt.replace_time_zone("UTC"))
shape: (1,)
Series: '' [bool]
[
        true
...
### Treat labels outside an `Enum` as absent in `is_in()` and `list.contains()`

Searching `String` values for an `Enum` needle, or `Enum` values for a `String` needle, used to cast
one side to the other's type, which raised as soon as any string, searched for or searched through,
was not a category of the `Enum`. Such a string now simply matches nothing. The same applies to
`arr.contains()`, and to `map.get()` and `map.contains_key()` on a Map with `Enum` keys.
...
```pycon
>>> values = pl.Series([["a", "z"]])
>>> values.list.contains(pl.lit("a", pl.Enum(["a", "b"])))
Traceback (most recent call last):
...
polars.exceptions.InvalidOperationError: conversion from `str` to `enum` failed in column '' for 1 out of 2 values: ["z"]
```
