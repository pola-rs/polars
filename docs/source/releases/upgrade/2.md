...
`bin_quantiles()` and `bin_ranks()`. The new methods return the integer bin index instead of a label
`Enum`.
...
- `labels` is required: pass one label per bin, or `labels=False` for the integer bin index. There
  is no equivalent of the interval strings (`"(-inf, -1]"`, ...) that `cut()`/`qcut()` generate by
  default. The reason for this is that `bin_*` methods work with arbitrary orderable data, not just
  numeric values.
