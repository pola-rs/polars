    ...
    pub fn execute(self) -> PolarsResult<QueryResult> {
            ...
            && std::env::var("POLARS_LOG_METRICS").as_deref() == Ok("1")
        {
            ...
            for phys_node_key in phys_sm.keys() {
                ...
                let io_total_active_recv_time =
                    Duration::from_nanos(node_metrics.io_total_active_recv_ns);
                ...
                lines.push(
                    (total_time, format!(
                        "{name}: tot({total_time:.2?}), \
                                 ...
                                 io_recv(total_active_time={io_total_active_recv_time:.2?}, total_bytes_requested={io_total_bytes_requested}, total_bytes_received={io_total_bytes_received})"))
                );
