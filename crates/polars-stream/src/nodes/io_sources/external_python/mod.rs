...
impl std::fmt::Debug for PythonFileReaderBuilder {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PythonFileReaderBuilder")
            ...
            .field("send_dfs_loop_threadpool", &self.send_dfs_loop_threadpool)
            .finish()
    }
}
        ...
        ASYNC.block_in_place(|| {
            Python::attach(|py| {
                ...
                let reader = self.builder().call_method1(
                    ...
                    (py_source, self.py_multi_scan_context.as_ref()),
                )?;
                ...
                PolarsResult::Ok(Box::new(PythonFileReader {
                    inner: Arc::new(PythonFileReaderInner {
                        ...
                        on_drop_called: AtomicBool::new(false),
                    }),
...
fn call_on_drop(inner: &PythonFileReaderInner) -> PolarsResult<()> {
    if inner
        .on_drop_called
        .compare_exchange(
            false,
        ...
        .is_ok()
    {
        ASYNC.block_in_place(|| {
            Python::attach(|py| {
                inner.reader.call_method0(py, intern!(py, "on_drop"))?;
                PolarsResult::Ok(())
            })
        })?;
    }
...
impl FileReader for PythonFileReader {
    ...
    fn prepare_read(&mut self) -> PolarsResult<()> {
        ASYNC.block_in_place(|| {
            Python::attach(|py| {
                self.inner
                    .reader
                    .call_method0(py, intern!(py, "initialize_serial"))?;
    ...
    fn begin_read(
        ...
        assert!(!self.begin_read_called); // call_on_drop() expects begin_read() only called once.
        self.begin_read_called = true;
            ...
            ASYNC
                .spawn_blocking(move || {
                    Python::attach(|py| {
                        ...
                        kwargs.set_item(intern!(py, "num_pipelines"), num_pipelines)?;
                        ...
                        slf.send_dfs_loop_threadpool.spawn_call(
                            py,
...
impl Drop for PythonFileReaderInner {
    fn drop(&mut self) {
        if let Err(x) = std::panic::catch_unwind(AssertUnwindSafe(|| call_on_drop(self).unwrap())) {
            if std::thread::panicking() {
                eprintln!("not raised (already panicking): PythonFileReader on_drop error: {x:?}")
            } else {
