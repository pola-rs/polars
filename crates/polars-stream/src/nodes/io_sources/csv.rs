...
const NO_SLICE: (usize, usize) = (0, 0);
...
struct LineBatch {
    ...
    bytes: &'static [u8],
    n_lines: usize,
    /// Note: A (0, 0) slice is used to indicate no-op.
    slice: (usize, usize),
...
struct InitializedState {
    // The mutex is for interior mutability to potentially update with a decompressed memslice.
    memslice: MemSlice,
}

impl InitializedState {
    /// Decompress the bytes, storing them back into `memslice`.
    fn decompress_if_needed(&mut self) -> PolarsResult<()> {
        let mut out = vec![];
        maybe_decompress_bytes(&self.memslice, &mut out)?;

        if !out.is_empty() {
            self.memslice = MemSlice::from_vec(out);
        }
...
impl FileReader for CsvFileReader {
    async fn initialize(&mut self) -> PolarsResult<()> {
        ...
        self.init_data = Some(InitializedState { memslice });
    ...
    fn begin_read(
        ...
        self.init_data.as_mut().unwrap().decompress_if_needed()?;

        let InitializedState { memslice } = self.init_data.as_ref().unwrap();
        ...
        let (mut inferred_schema, ..) = polars_io::csv::read::infer_file_schema(
            ...
            &mut self.options.n_threads.clone(),
        )?;

        // Currently always override with the projected dtype.
        // TODO: This can be improved to better choose between the 2 possible dtypes.
...
impl ChunkReader {
    fn try_new(
        ...
        if let Some(dtypes) = options.dtype_overwrite.as_deref() {
            let s = Arc::make_mut(&mut reader_schema);
