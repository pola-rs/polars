...
impl BufferedStream {
    pub fn new(morsels: Vec<Morsel>, start_offset: MorselSeq) -> PolarsResult<Self> {
        ...
        for mut morsel in morsels {
            morsel.set_seq(seq);
            let token = mm().store_sync(morsel.into_df())?;
    ...
    fn drop(&mut self) {
        POOL.install(|| {
            ...
            (0..self.morsels.len()).into_par_iter().for_each(|_| {
                if let Some((token, _)) = self.morsels.pop() {
                    let _ = mm().drop_sync(token);
                }
            });
        })
    }
