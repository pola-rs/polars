        ...
        v @ IR::Scan { .. } => {
            ...
            if scan_sources.is_empty() || file_options.pre_slice.is_some_and(|(_, len)| len == 0) {
                ...
                let mut multi_scan_node = PhysNodeKind::MultiScan {
                    ...
                    file_reader_builder: Some(file_reader_builder),
                ...
                *predicate_to_multiscan = predicate_post.take();
                // * Push negative slice as multiscan internally resolves it.
                //   * TODO: This could maybe use an ReaderCapabilities::FAST_ROW_COUNT.
                // TODO: Do not push negative slice for oneshot readers that do not support row
                // counts (IO plugins). This also means we cannot push predicates in that case.
                *pre_slice_to_multiscan = pre_slice_post.take();
                ...
                // TODO: todos for updated multiscan
                // * If we have row index with positive slice, put row index to a separate streaming node after adjusting
                //   the offset, and apply the slice first. Then enable the debug assertion in the multi scan.
                // * Also use <dyn FileReaderBuilder>::reader_capabilities() here to determine which operations to rewrite to
                //   nodes.
                // * Remove MultiScanRowRestriction in favor of slice_enum::Slice.
                // * Remove logic for single file below, always dispatch to MultiScan.
                ...
                let mut node = PhysNodeKind::MultiScan {
                    ...
                    file_reader_builder: None,
                    cloud_options: None,
                    projected_file_schema: Default::default(),
                    pre_slice: None,
                };
