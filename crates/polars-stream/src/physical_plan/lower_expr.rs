    ...
    for expr in exprs.iter().copied() {
        ...
        match ctx.expr_arena.get(expr).clone() {
            ...
            AExpr::Function {
                ...
                output_schema.insert(out_name.clone(), DataType::IDX_DTYPE);
                let kind = PhysNodeKind::WithRowIndex {
                    input,
                    name: out_name.clone(),
                    offset: None
                };
                let with_row_idx_node_key = ctx.phys_sm.insert(PhysNode::new(Arc::new(output_schema), kind));
                let row_idx_col_aexpr = ctx.expr_arena.add(AExpr::Column(out_name.clone()));
                let row_idx_col_expr_ir = ExprIR::new(row_idx_col_aexpr, OutputName::ColumnLhs(out_name));
                let row_idx_stream = build_select_stream_with_ctx(PhysStream::first(with_row_idx_node_key), &[row_idx_col_expr_ir], ctx)?;
