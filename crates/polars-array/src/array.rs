...
mod tests {
    ...
    /// The shape a [`PlArrayType`] does not name is the one the array asked for carries.
    #[test]
    fn new_full_null_keeps_the_shape_of_the_array() {
        for arr in arrays() {
        ...
        assert_eq!(array.fields()[0].len(), 5);
        assert_eq!(array.fields()[0].null_count(), 5);
    }
