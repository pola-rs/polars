...
impl<V: ViewType + ?Sized> BinaryViewArrayGenericBuilder<V> {
    /// Spec cap on `offset + length` per view (signed i32).
    ///
    /// Rows longer than this take a dedicated buffer at `offset = 0`,
    /// so we still support rows up to [`Self::MAX_ROW_BYTE_LEN`].
    pub const MAX_BUFFER_LEN: usize = i32::MAX as usize;

    /// Per-row length cap. `u32::MAX` is reserved; largest valid row is
    /// `u32::MAX - 1`.
    ...
    fn reserve_active_buffer(&mut self, additional: usize) {
        ...
        // Second test is overflow-safe: `additional` may exceed `MAX_BUFFER_LEN`.
        if additional > cap - len || additional > Self::MAX_BUFFER_LEN - len {
    ...
    fn reserve_active_buffer_slow(&mut self, additional: usize) {
        ...
        // `.max(additional)` lets oversize rows live in their own buffer at offset 0.
...
mod tests {
    ...
    #[test]
    fn non_inline_sequence_roundtrips() {
        ...
        b.push_value_ignore_validity(b"second non-inline string here");
        b.push_value_ignore_validity(b"third non-inline string here");
        let arr = b.freeze();
        ...
        assert_eq!(arr.value(1), b"second non-inline string here");
        assert_eq!(arr.value(2), b"third non-inline string here");
    }
}
