...
/// converts a `i64` representing a `timestamp(s)` to [`DateTime`]
///
/// Computed via pure calendar arithmetic; see [`date_to_date32_opt`] for why this
/// avoids going through a [`jiff::Timestamp`].
#[inline]
...
pub fn timestamp_ms_to_datetime_opt(v: i64) -> Option<DateTime> {
    unix_epoch_date()
        .to_datetime(Time::midnight())
        .checked_add(SignedDuration::from_millis(v))
        .ok()
}
...
pub fn timestamp_us_to_datetime_opt(v: i64) -> Option<DateTime> {
    unix_epoch_date()
        .to_datetime(Time::midnight())
        .checked_add(SignedDuration::from_micros(v))
        .ok()
}
...
pub fn timestamp_ns_to_datetime_opt(v: i64) -> Option<DateTime> {
    unix_epoch_date()
        .to_datetime(Time::midnight())
        .checked_add(SignedDuration::from_nanos(v))
        .ok()
}
...
///
/// This is computed via pure calendar arithmetic ([`Date::duration_since`]) rather than
/// by going through a [`jiff::Timestamp`]: `Timestamp`'s range is deliberately narrower
/// than `Date`'s (it's bounded such that it remains valid under any legal UTC offset),
/// so routing through it would spuriously reject valid dates near the end of `Date`'s
/// own range (e.g. `9999-12-31`).
#[inline]
pub fn date_to_date32_opt(date: Date) -> Option<i32> {
    let secs = date.duration_since(unix_epoch_date()).as_secs();
    i32::try_from(secs / SECONDS_IN_DAY).ok()
}
...
pub fn datetime_to_epoch_nanos_opt(dt: DateTime) -> Option<i128> {
    let epoch = unix_epoch_date().to_datetime(Time::midnight());
    Some(dt.duration_since(epoch).as_nanos())
}
...
pub fn epoch_nanos_to_datetime_opt(epoch_nanos: i128) -> Option<DateTime> {
    let epoch = unix_epoch_date().to_datetime(Time::midnight());
    let dur = SignedDuration::try_from_nanos_i128(epoch_nanos)?;
    epoch.checked_add(dur).ok()
}
...
pub fn utf8_to_timestamp_scalar(
    ...
    timestamp_to_timeunit_scalar(ts, tu)
}

/// Parses an ISO 8601 / RFC 3339 timestamp string via jiff's native
/// [`Timestamp`] parser (which, unlike jiff's strtime engine, honors the
/// string's own embedded offset or `Z`). Returns in scale `tu`.
#[inline]
pub fn iso8601_to_timestamp_scalar(value: &str, tu: &TimeUnit) -> Option<i64> {
    let ts = value.parse::<Timestamp>().ok()?;
    timestamp_to_timeunit_scalar(ts, tu)
}

#[inline]
fn timestamp_to_timeunit_scalar(ts: Timestamp, tu: &TimeUnit) -> Option<i64> {
    Some(match tu {
