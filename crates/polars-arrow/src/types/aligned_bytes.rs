...
pub trait SameBytes<B: AlignedBytes>: Pod + sealed::Sealed {}
...
pub trait AlignedBytes: Pod + Zeroable + Copy + Default + Eq {
    ...
    fn cast_vec_ref_mut<T: SameBytes<Self>>(vec: &mut Vec<T>) -> &mut Vec<Self> {
...
macro_rules! impl_aligned_bytes {
    (
        ...
        $(
        impl sealed::Sealed for $eq_type {}
        impl SameBytes<$name> for $eq_type {}
        ...
        impl Into<$eq_type> for $name {
            #[inline(always)]
            fn into(self) -> $eq_type {
