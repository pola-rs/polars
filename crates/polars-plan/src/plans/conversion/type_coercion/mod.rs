...
use datetime::{ensure_datetime, ensure_int, resolve_datetime_dtype};
            ...
            AExpr::Function {
                ...
                let (from_types, to_types) = match arg_type {
                    DateRangeArgs::StartEndInterval => {
                        ...
                        ensure_datetime!(start);
                        ensure_datetime!(end);
                        let initial_st = get_supertype(&start, &end).unwrap();
                        let supertype = resolve_datetime_dtype(initial_st, tu, tz, interval)?;
                        let from_types = vec![start, end];
                    ...
                    DateRangeArgs::StartEndSamples => {
                        ...
                        ensure_datetime!(end);
                        ensure_int!(num_samples);
                        let initial_st = unpack!(get_supertype(&start, &end));
                        let supertype = resolve_datetime_dtype(initial_st, tu, tz, interval)?;
                        let from_types = vec![start, end, num_samples];
                    ...
                    DateRangeArgs::StartIntervalSamples => {
                        ...
                        ensure_datetime!(start);
                        ensure_int!(num_samples);
                        let supertype = resolve_datetime_dtype(start.clone(), tu, tz, interval)?;
                        let from_types = vec![start, num_samples];
                    ...
                    DateRangeArgs::EndIntervalSamples => {
                        ...
                        ensure_datetime!(end);
                        ensure_int!(num_samples);
                        let supertype = resolve_datetime_dtype(end.clone(), tu, tz, interval)?;
                        let from_types = vec![end, num_samples];
                        let to_types = vec![supertype, DataType::Int64];
                        (from_types, to_types)
                    },
