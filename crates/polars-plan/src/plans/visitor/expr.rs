        ...
        loop {
            match (scratch1.pop(), scratch2.pop()) {
                (Some(l), Some(r)) => {
                    ...
                    scratch1.extend(l.to_aexpr().children_iter_name_last());
                    scratch2.extend(r.to_aexpr().children_iter_name_last());
                },
...
impl TreeWalker for AexprNode {
    ...
    fn apply_children<F: FnMut(&Self, &Self::Arena) -> PolarsResult<VisitRecursion>>(
        ...
        scratch.extend(self.to_aexpr(arena).children_iter_name_last());
    ...
    fn map_children<F: FnMut(Self, &mut Self::Arena) -> PolarsResult<Self>>(
        ...
        scratch.extend(ae.children_iter_name_last());
        ...
        for (l, r) in ae.children_iter_mut_name_last().zip_eq(scratch) {
            *l = r;
        }
