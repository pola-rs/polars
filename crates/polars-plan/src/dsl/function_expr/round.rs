...
pub(super) fn round(c: &Column, decimals: u32) -> PolarsResult<Column> {
    match c {
        ...
        Column::Scalar(s) if s.is_empty() => s.as_materialized_series().round(decimals).map(Column::from),
...
pub(super) fn round_sig_figs(c: &Column, digits: i32) -> PolarsResult<Column> {
    match c {
        ...
        Column::Scalar(s) if s.is_empty() => s.as_materialized_series().round_sig_figs(digits).map(Column::from),
        Column::Scalar(s) => ScalarColumn::from_single_value_series(
