...
pub(super) fn corr(
    ...
    match method {
        ...
        CorrelationMethod::Covariance => covariance(s, ddof, min_periods),
    }
}
...
fn covariance(s: &[Series], ddof: u8, min_periods: usize) -> PolarsResult<Series> {
    ...
    let ret = match a.dtype() {
        DataType::Float32 => {
            let ret = cov(a.f32().unwrap(), b.f32().unwrap(), ddof, min_periods).map(|v| v as f32);
            return Ok(Series::new(name, &[ret]));
        },
        DataType::Float64 => cov(a.f64().unwrap(), b.f64().unwrap(), ddof, min_periods),
        DataType::Int32 => cov(a.i32().unwrap(), b.i32().unwrap(), ddof, min_periods),
        DataType::Int64 => cov(a.i64().unwrap(), b.i64().unwrap(), ddof, min_periods),
        DataType::UInt32 => cov(a.u32().unwrap(), b.u32().unwrap(), ddof, min_periods),
        DataType::UInt64 => cov(a.u64().unwrap(), b.u64().unwrap(), ddof, min_periods),
        _ => {
            let a = a.cast(&DataType::Float64)?;
            let b = b.cast(&DataType::Float64)?;
            cov(a.f64().unwrap(), b.f64().unwrap(), ddof, min_periods)
        },
    };
...
fn pearson_corr(s: &[Series], ddof: u8, min_periods: usize) -> PolarsResult<Series> {
    ...
    let ret = match a.dtype() {
        DataType::Float32 => {
            let ret = pearson_corr(a.f32().unwrap(), b.f32().unwrap(), ddof, min_periods)
                .map(|v| v as f32);
            return Ok(Series::new(name, &[ret]));
        },
