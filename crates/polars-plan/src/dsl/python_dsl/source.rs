...
impl PythonOptionsDsl {
    pub fn get_scan_fn(&self) -> PolarsResult<&PythonFunction> {
        self.scan_fn
            .as_ref()
            .ok_or_else(|| polars_err!(ComputeError: "PythonOptionsDsl::scan_fn is None"))
    }

    // pub fn take_scan_fn(&mut self) -> PolarsResult<PythonFunction> {
    //     self.scan_fn
    //         .take()
    //         .ok_or_else(|| polars_err!(ComputeError: "PythonOptionsDsl::scan_fn is None"))
    // }
