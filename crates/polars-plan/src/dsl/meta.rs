...
impl MetaNameSpace {
    ...
    pub fn pop(self, schema: Option<&Schema>) -> PolarsResult<Vec<Expr>> {
        ...
        let expr = to_expr_ir(self.0, &mut arena, &Default::default(), schema)?;
    ...
    pub fn is_simple_projection(&self, schema: Option<&Schema>) -> bool {
        ...
        let mut arena = Arena::with_capacity(8);
        to_expr_ir(self.0.clone(), &mut arena, &Default::default(), schema)
    ...
    pub fn into_tree_formatter(
        ...
        let node = to_expr_ir(self.0, &mut arena, &Default::default(), schema)?.node();
