...
impl DslPlan {
    ...
    pub fn bind(self, bindings: &PlHashMap<PlSmallStr, DslPlan>) -> PolarsResult<Self> {
        ...
        match self {
            ...
            lp @ Scan { .. } | lp @ DataFrameScan { .. } => Ok(lp),
            #[cfg(feature = "python")]
            lp @ PythonScan { .. } => Ok(lp),

            // Single-input nodes.
            Filter { input, predicate } => Ok(Filter {
                input: bind_arc(input, bindings)?,
                predicate,
            }),
            Cache { input, id } => Ok(Cache {
                input: bind_arc(input, bindings)?,
                id,
            }),
            Select {
            ...
            PipeWithSchema { input, callback } => Ok(PipeWithSchema {
                input: Arc::from(
                    input
                        .iter()
                        .cloned()
            ...
            Distinct { input, options } => Ok(Distinct {
                input: bind_arc(input, bindings)?,
                options,
            }),
            Sort {
            ...
            Slice { input, offset, len } => Ok(Slice {
                input: bind_arc(input, bindings)?,
                offset,
                len,
            }),
            MapFunction { input, function } => Ok(MapFunction {
                input: bind_arc(input, bindings)?,
                function,
            }),
            Sink { input, payload } => Ok(Sink {
                input: bind_arc(input, bindings)?,
                payload,
            }),
            #[cfg(feature = "pivot")]
            ...
            Join {
                ...
                input_left: bind_arc(input_left, bindings)?,
                input_right: bind_arc(input_right, bindings)?,
                left_on,
                right_on,
                predicates,
                options,
            ...
            HConcat { inputs, options } => Ok(HConcat {
                inputs: inputs
                    .into_iter()
                    .map(|p| p.bind(bindings))
                    .collect::<PolarsResult<_>>()?,
            ...
            SinkMultiple { inputs } => Ok(SinkMultiple {
                inputs: inputs
                    .into_iter()
                    .map(|p| p.bind(bindings))
                    .collect::<PolarsResult<_>>()?,
            }),
            ExtContext { input, contexts } => Ok(ExtContext {
                input: bind_arc(input, bindings)?,
                contexts: contexts
                    .into_iter()
                    .map(|p| p.bind(bindings))
            ...
            // IR wrapper node.
            IR { dsl, version, node } => Ok(IR {
                dsl: bind_arc(dsl, bindings)?,
                version,
                node,
            }),
        }
