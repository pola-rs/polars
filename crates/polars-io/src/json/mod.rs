    ...
    fn finish(&mut self, df: &mut DataFrame) -> PolarsResult<()> {
        ...
        let fields = df
            ...
            .map(|s| {
                ...
                Ok(s.field().to_arrow(PlFlavor::highest()))
            })
            .collect::<PolarsResult<Vec<_>>>()?;
        let batches = df
            .iter_chunks(PlFlavor::highest(), false)
    ...
    W: Write,
{
    ...
    pub fn write_batch(&mut self, df: &DataFrame) -> PolarsResult<()> {
        let fields = df
            ...
            .collect::<PolarsResult<Vec<_>>>()?;
        let chunks = df.iter_chunks(PlFlavor::highest(), false);
        let batches =
        ...
        let out = match self.json_format {
            JsonFormat::Json => {
                ...
                let dtype = if let Some(mut schema) = self.schema {
                    ...
                    DataType::Struct(schema.iter_fields().collect()).to_arrow(PlFlavor::highest())
                } else {
                    // infer
