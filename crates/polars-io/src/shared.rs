...
use polars_core::prelude::{ArrowField, ArrowSchema, DataFrame, IdxSize, Schema, Series};
...
pub(crate) fn schema_to_arrow_checked(
    ...
    let fields = schema.iter_fields().map(|field| {
        #[cfg(feature = "object")]
        polars_ensure!(!matches!(field.data_type(), DataType::Object(_, _)), ComputeError: "cannot write 'Object' datatype to {}", file_name);
