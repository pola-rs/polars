...
pub use self::binary_to::{binary_to_binview, binary_to_list, fixed_size_binary_to_binview};
#[cfg(feature = "dtype-decimal")]
pub use self::binview_to::binview_to_decimal;
pub use self::binview_to::{
...
#[cfg(feature = "dtype-decimal")]
pub use self::primitive_to::decimal_to_utf8view;
pub use self::primitive_to::{
...
/// The Polars type whose values `array` holds, which is the physical type of a logical one.
...
fn cast_dispatch(
    ...
    if matches!(from, D::Boolean) {
        ...
        return match to {
            D::String => Ok(Box::new(boolean_to::boolean_to_utf8view(array))),
            D::Binary => Ok(Box::new(boolean_to::boolean_to_binaryview(array))),
    ...
    if matches!(to, D::Boolean) {
        ...
        if matches!(from, D::Decimal(_, _)) {
            return Ok(Box::new(primitive_to::primitive_to_boolean::<i128>(
                downcast(array),
            )));
        }
    ...
    if let D::Decimal(_, from_scale) = from {
        ...
        return match to {
            D::Decimal(to_precision, to_scale) => {
                ...
                Ok(Box::new(primitive_to::decimal_to_decimal(
                    array,
                    *from_precision,
                    from_scale,
                    *to_precision,
            ...
            D::String => Ok(Box::new(primitive_to::decimal_to_utf8view(
                array, from_scale,
            ))),
            _ => unsupported(from, to),
        };
...
fn cast_temporal(
    ...
    match (from, to) {
        ...
        | (D::Duration(from_unit), D::Duration(to_unit)) => {
            Ok(Box::new(primitive_to::rescale_time(
                downcast(array),
                time_unit_multiple(*from_unit),
                time_unit_multiple(*to_unit),
            )))
        },
        (D::Datetime(from_unit, _), D::Date) => Ok(Box::new(primitive_to::timestamp_to_date(
            downcast(array),
...
fn cast_string(
    ...
    match to {
        ...
        D::BinaryOffset => Ok(Box::new(binview_to::view_to_binary(
            &array.clone().into_binview(),
        ))),
        #[cfg(feature = "dtype-decimal")]
        D::Decimal(precision, scale) => Ok(Box::new(binview_to::binview_to_decimal(
            &array.clone().into_binview(),
            *precision,
            *scale,
        ))),
...
fn cast_bytes(
    ...
    let view: &PlBinaryViewArray = match from {
        ...
        _ => {
            converted = binary_to::binary_to_binview(downcast::<PlBinaryArray>(array));
            &converted
        },
    };
...
fn cast_nested(
    ...
    let out: Box<dyn PlArray> = match (from, to) {
        ...
        (D::Binary, D::List(inner)) if matches!(**inner, D::UInt8) => {
            Box::new(binary_to::binary_to_list(&binview_to::view_to_binary(
...
/// Whether the values are cast with `as`, saturating rather than reading as null.
...
fn time_unit_multiple(unit: polars_dtype::TimeUnit) -> i64 {
    temporal::time_unit_multiple(unit.to_arrow())
}
