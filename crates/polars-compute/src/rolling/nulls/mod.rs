    ...
    Fo: Fn(Idx, WindowSize, Len) -> (Start, End) + Copy,
{
    ...
    Box::new(PlPrimitiveArray::from_vec(out).with_validity(Some(validity.into())))
}
...
mod test {
    ...
    fn test_rolling_extrema_nulls() {
        ...
        let validity = PlBitmap::new_scalar(true, vals.len());
        let window_size = 3;
        let min_periods = 3;
        ...
        assert_eq!(
            &elements_of::<i32>(&*out)[2..],
            &[3, 10, 10, 10, 10, 10, 9, 8, 7, 6, 5, 4, 3].map(Some),
        );
    }
}
