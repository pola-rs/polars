...
mod tests {
    ...
    fn test_rolling_argmin_by_basic() {
        ...
        assert_eq!(actual, vec![Some(0), Some(1), Some(1), Some(3), Some(3)]);
    }
    ...
    fn test_rolling_argmax_by_basic() {
        ...
        let actual: Vec<Option<u32>> = result.iter().map(|v| v.copied()).collect();
        assert_eq!(actual, vec![Some(0), Some(0), Some(0), Some(2), Some(2)]);
    }
    ...
    fn test_rolling_argmin_by_min_periods() {
        ...
        let actual: Vec<Option<u32>> = result.iter().map(|v| v.copied()).collect();
        assert_eq!(actual, vec![None, None, Some(1), Some(3), Some(3)]);
    }
