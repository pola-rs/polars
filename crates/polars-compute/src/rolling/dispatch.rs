...
//!
//! Each entry point here takes a `PlPrimitiveArray` as it stands and resolves, once, the two
//! things the implementations below need settled — whether the buffers hold one slot per element,
//! and whether any element is null. It then hands the chunk to [`no_nulls`] or [`nulls`], whose
//! signatures state which of the two they were written for.
//!
//! Callers reach for these rather than for [`no_nulls`]/[`nulls`] directly, so that the dispatch
//! is written once and a chunk is never written out on the way to a kernel that could have read it
//! as it was.
...
use super::{RollingFnParams, no_nulls, nulls, quantile_filter, rolling_chunk};
