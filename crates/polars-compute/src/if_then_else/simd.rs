...
// The AVX-512 kernels get autogenerated by inlining the routine into the
// functions with the appropriate target_feature enabled.
...
unsafe fn with_avx512<R>(f: impl FnOnce() -> R) -> R {
    f()
}

fn with_avx512_if_available<R>(f: impl FnOnce() -> R) -> R {
    #[cfg(target_arch = "x86_64")]
    if is_avx512_enabled() && std::arch::is_x86_feature_detected!("avx512bw") {
        return unsafe { with_avx512(f) };
    }
