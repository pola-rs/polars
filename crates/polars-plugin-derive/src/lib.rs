...
use quote::quote;
use syn::{FnArg, Ident, parse_macro_input};

/// Resolve the runtime crate from the dependencies of the crate invoking the macro.
fn find_crate(package: &str) -> Option<proc_macro2::TokenStream> {
...
fn runtime_crate() -> syn::Result<proc_macro2::TokenStream> {
    find_crate("polars-plugin")
        .or_else(|| find_crate("pyo3-polars"))
        .ok_or_else(|| {
            syn::Error::new(
                Span::call_site(),
                "`polars_expr` requires a direct dependency on either `polars-plugin` or `pyo3-polars`",
            )
        })
}

fn quote_get_kwargs(krate: &proc_macro2::TokenStream) -> proc_macro2::TokenStream {
    quote!(
    let kwargs = std::slice::from_raw_parts(kwargs_ptr, kwargs_len);

    let kwargs = match #krate::derive::_parse_kwargs(kwargs)  {
        Ok(value) => value,
        Err(err) => {
            let err = polars_error::polars_err!(InvalidOperation: "could not parse kwargs: '{}'\n\nCheck: registration of kwargs in the plugin.", err);
            #krate::derive::_update_last_error(err);
            return;
        }
    };
...
fn quote_call_kwargs(
    ast: &syn::ItemFn,
    fn_name: &syn::Ident,
    krate: &proc_macro2::TokenStream,
) -> proc_macro2::TokenStream {
    let kwargs = quote_get_kwargs(krate);
    quote!(
...
fn quote_process_results(krate: &proc_macro2::TokenStream) -> proc_macro2::TokenStream {
    quote!(match result {
        Ok(out) => {
            // Update return value.
...
fn create_expression_function(
    ...
    let quote_process_result = quote_process_results(krate);
    ...
    quote! (
        ...
        pub unsafe extern "C" fn #map_field_name(
            ...
            unsafe {
                ...
                // Equivalent to `FieldsMapper::new(&inputs).with_dtype(dtype)`,
                // inlined to keep the runtime free of the polars-plan dependency.
                let dtype = polars_core::datatypes::DataType::#dtype;
                let out = polars_core::prelude::Field::new(inputs[0].name().clone(), dtype);
                let out = polars_arrow::ffi::export_field_to_c(&out.to_arrow(polars_core::datatypes::CompatLevel::newest()));
                *return_value = out;
            }
        }
...
pub fn polars_expr(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded_field_fn = if let Some(fn_name) = options.output_type_fn {
        create_field_function(&ast.sig.ident, &fn_name, false, &krate)
    } else if let Some(fn_name) = options.output_type_fn_kwargs {
        create_field_function(&ast.sig.ident, &fn_name, true, &krate)
    ...
    let expanded_expr = create_expression_function(ast, &krate);
    let expanded = quote!(
        #expanded_field_fn
