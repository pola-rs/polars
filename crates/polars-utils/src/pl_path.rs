...
impl PlPath {
    ...
    pub fn to_absolute_path(&self) -> PlRefPath {
        PlRefPath::try_from_pathbuf(std::path::absolute(Path::new(self.strip_scheme())).unwrap())
            .unwrap()
    }
...
impl PlRefPath {
    ...
    pub fn to_absolute_path(&self) -> Cow<'_, PlRefPath> {
        if self.has_scheme() || self.as_std_path().is_absolute() {
            Cow::Borrowed(self)
        } else {
