...
fn replace_n<'a>(
    ...
    // Fast path: all scalar (length-1). Avoids per-row regex compilation.
    ...
    // Fast path: scalar pattern with multi-value replacement.
    // Compile regex once outside the loop.
    if pat.len() == 1 && ca.len() > 1 {
        if n > 1 {
        ...
        let lit = is_literal_pat(&pat_str);
        ...
        return Ok(iter_and_replace(ca, val, f));
    }

    // General 3-way broadcast: any of ca / pat / val may be length-1.
    // Determine the broadcast length (max non-1 length).
    let len_ca = ca.len();
    let len_pat = pat.len();
    let len_val = val.len();

    let broadcast_len = [len_ca, len_pat, len_val]
        .iter()
        .copied()
        .filter(|&l| l != 1)
        .max()
    ...
    for (name, len) in [
        ("string column", len_ca),
        ("pattern", len_pat),
        ("value", len_val),
    ] {
        polars_ensure!(
            len == 1 || len == broadcast_len,
            ComputeError:
            "{} length ({}) is not compatible with broadcast length ({})",
            name, len, broadcast_len,
        );
    }

    if n == 0 {
        // Return a broadcast-length clone of ca.
        if len_ca == broadcast_len {
            return Ok(ca.clone());
        }
        // ca is length-1; broadcast to broadcast_len by repeating.
        let mut out: StringChunked = (0..broadcast_len).map(|_| ca.get(0)).collect_trusted();
        out.rename(ca.name().clone());
        return Ok(out);
    }

    // Per-row replacement with full broadcast semantics.
    let mut out: StringChunked = (0..broadcast_len)
        .map(|i| {
            let opt_src = ca.get(if len_ca == 1 { 0 } else { i });
            let opt_pat = pat.get(if len_pat == 1 { 0 } else { i });
            let opt_val = val.get(if len_val == 1 { 0 } else { i });
