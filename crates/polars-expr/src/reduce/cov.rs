...
use polars_plan::plans::IRRegressionFunction;
...
pub fn new_regression_reduction(
    dtype_x: DataType,
    dtype_y: DataType,
    function: IRRegressionFunction,
) -> PolarsResult<Box<dyn GroupedReduction>> {
    polars_ensure!(
        dtype_x.is_primitive_numeric(),
        InvalidOperation: "`{function}` operation not supported for dtype `{dtype_x}`"
    );
    polars_ensure!(
        dtype_y.is_primitive_numeric(),
        InvalidOperation: "`{function}` operation not supported for dtype `{dtype_y}`"
    );
    Ok(Box::new(RegressionGroupedReduction {
        values: Vec::new(),
        evicted_values: Vec::new(),
        function,
    }))
...
// Inputs are received in [x, y] order, where x is the independent and y the
// dependent variable.
struct RegressionGroupedReduction {
    values: Vec<PearsonState>,
    evicted_values: Vec<PearsonState>,
    function: IRRegressionFunction,
}

impl GroupedReduction for RegressionGroupedReduction {
    fn new_empty(&self) -> Box<dyn GroupedReduction> {
        Box::new(Self {
            values: Vec::new(),
            evicted_values: Vec::new(),
    ...
    fn finalize(&mut self) -> PolarsResult<Series> {
        use IRRegressionFunction as R;
        let v = core::mem::take(&mut self.values);
        match self.function {
            R::Count => {
                let ca: IdxCa = v
                    .into_iter()
                    .map(|s| s.count() as IdxSize)
                    .collect_ca(PlSmallStr::EMPTY);
                Ok(ca.into_series())
            },
            function => {
                let ca: Float64Chunked = v
                    .into_iter()
                    .map(|s| match function {
                        R::Slope => s.regr_slope(),
                        R::Intercept => s.regr_intercept(),
                        R::R2 => s.regr_r2(),
                        R::Count => unreachable!(),
                    })
                    .collect_ca(PlSmallStr::EMPTY);
