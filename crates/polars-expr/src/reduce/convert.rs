...
use crate::reduce::first_last_single::{
    new_first_reduction, new_last_reduction, new_single_reduction,
};
...
pub fn into_reduction(
    ...
    let out = match expr_arena.get(node) {
        AExpr::Agg(agg) => match agg {
            ...
            IRAggExpr::Single(input) => (new_single_reduction(get_dt(*input)?), *input),
            IRAggExpr::Count {
                input,
                include_nulls,
            } => {
