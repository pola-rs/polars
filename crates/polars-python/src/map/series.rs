...
fn infer_and_finish<'py, A: ApplyLambda<'py>>(
    ...
    if out.is_instance_of::<PyBool>() {
        ...
        let series = py_pyseries.extract::<PySeries>().unwrap().series.into_inner();
        let dt = series.dtype();
        applyer
        ...
        let series = py_pyseries.extract::<PySeries>(py).unwrap().series.into_inner();
    ...
    T: IntoPyObject<'py>,
{
    ...
    if out.is_none() {
        ...
        py_series.extract::<PySeries>().map(|s| Some(s.series.into_inner()))
    }
}
...
fn call_series_lambda(
    ...
    Ok(py_pyseries.extract::<PySeries>().ok().map(|s| s.series.into_inner()))
}
