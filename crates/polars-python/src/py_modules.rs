...
static PYSERIES: GILOnceCell<PyObject> = GILOnceCell::new();
...
pub(crate) fn pyseries(py: Python<'_>) -> &PyObject {
    POLARS_RS.get_or_init(py, || polars_rs(py).getattr(py, "PySeries").unwrap())
}
