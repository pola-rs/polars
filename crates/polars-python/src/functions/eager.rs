...
///
/// This is a fast option for `pl.repeat(..., eager=True)`, it bypasses the query
/// engine and materializes the Series directly, which reduces overhead
/// for simple queries.
///
/// NOTE: `value` must be an `Expr::Literal` holding a *scalar* literal. Note that
/// list/array/struct literals are scalars in this sense. `Series` and `Range`
/// literals are not.
///
/// ## Example
///
///
/// ```python
/// # A scalar value with a known length can be evaluated fast
/// pl.repeat(42, n=420, dtype=pl.Int8, eager=True)
///
/// # n is only known once col("a") has been evaluated, thus this fast function should not be used
/// df.select(pl.repeat(42, n=pl.col("a").first()))
/// ```
#[pyfunction]
