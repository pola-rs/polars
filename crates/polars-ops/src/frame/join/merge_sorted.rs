...
fn _make_comparator<T>(descending: bool, nulls_last: bool) -> impl Fn(Option<T>, Option<T>) -> bool
where
...
fn series_to_merge_indicator(
    ...
    if lhs.dtype().is_nested() {
        return Ok(get_merge_indicator(
            lhs.row_encode_ordered(descending, nulls_last)?.iter(),
            rhs.row_encode_ordered(descending, nulls_last)?.iter(),
            false,
            false,
        ));
    }
