...
pub(crate) fn build_tables<T, I>(
    keys: Vec<I>,
    join_nulls: bool,
) -> Vec<PlHashMap<<T as IntoTotalOrd>::TotalOrdItem, IdxVec>>
where
    T: Send + Sync + TotalHash + TotalEq + IntoTotalOrd,
    <T as IntoTotalOrd>::TotalOrdItem: Send + Sync + Hash + Eq + Copy + DirtyHash + IsNull,
