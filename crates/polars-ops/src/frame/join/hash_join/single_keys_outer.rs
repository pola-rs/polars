...
where
    ...
    T: TotalHash + TotalEq + Send + IntoTotalOrd,
    <T as IntoTotalOrd>::TotalOrdItem: Send + Hash + Eq,
{
...
pub(crate) fn prepare_hashed_relation_threaded<T, I>(
    iters: Vec<I>,
) -> Vec<PlHashMap<<T as IntoTotalOrd>::TotalOrdItem, (bool, IdxVec)>>
where
    I: Iterator<Item = T> + Send + TrustedLen,
    T: TotalHash + TotalEq + Copy + Sync + Send + IntoTotalOrd,
    <T as IntoTotalOrd>::TotalOrdItem: Hash + Eq + Copy + Sync + Send,
{
...
fn probe_outer<T, F, G, H>(
    ...
    hash_tbls: &mut [PlHashMap<<T as IntoTotalOrd>::TotalOrdItem, (bool, IdxVec)>],
    results: &mut (
    ...
    T: TotalHash + TotalEq + Copy + Sync + Send + IsNull + IntoTotalOrd,
    <T as IntoTotalOrd>::TotalOrdItem: Hash + Eq + Copy + Sync + Send + IsNull,
    // idx_a, idx_b -> ...
