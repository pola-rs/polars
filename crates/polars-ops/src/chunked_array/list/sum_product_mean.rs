...
pub(super) fn product_with_nulls(ca: &ListChunked, inner_dtype: &DataType) -> PolarsResult<Series> {
    ...
    let mut out = match inner_dtype {
        ...
        UInt64 => {
            let out: UInt64Chunked = ca.try_apply_amortized_generic(|s| {
                s.map(|s| {
                    let scalar = s.as_ref().product()?;
                    match scalar.value() {
        ...
        Float32 => {
            let out: Float32Chunked = ca.try_apply_amortized_generic(|s| {
                s.map(|s| {
                    let scalar = s.as_ref().product()?;
                    match scalar.value() {
        ...
        Float64 => {
            let out: Float64Chunked = ca.try_apply_amortized_generic(|s| {
                s.map(|s| {
                    let scalar = s.as_ref().product()?;
                    match scalar.value() {
