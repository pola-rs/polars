...
use super::{any_value, TimeUnit, DataType};
...
pub fn anyvalue_strategy(
    ...
    (0..num_possible_types).prop_flat_map(move |i| {
        ...
        match selection {
            ...
            _ if selection == S::BOOLEAN => any::<bool>().prop_map(AnyValue::Boolean),
            _ if selection == S::STRING => ".*".prop_map(|s| AnyValue::String(s.as_str())).boxed(),
            _ if selection == S::STRING_OWNED => any::<String>().prop_map(|s| AnyValue::StringOwned(PlSmallStr::from_string(s))).boxed(),
            ...
            _ if selection == S::BINARY => prop::collection::vec(any<u8>(), options.vector_length_range.clone()).prop_map(|vec| {
                let leaked: &'static [u8] = Box::leak(vec.into_boxed_slice());
                AnyValue::Binary(leaked)
            }).boxed(),
            _ if selection == S::BINARY_OWNED => prop::collection::vec(any::<u8>(), options.vector_length_range.clone()).prop_map(AnyValue::BinaryOwned).boxed(),
            #[cfg(feature = "object")]
