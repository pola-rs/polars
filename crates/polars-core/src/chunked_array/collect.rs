...
pub trait ChunkedCollectIterExt<T: PolarsDataType>: Iterator + Sized {
        ...
        T::Array: ArrayFromIterDtype<Self::Item>,
    {
        ...
        let arr = self.collect_arr_with_dtype(field.dtype.to_arrow(PlFlavor::highest()));
    ...
    {
        let field = Arc::new(Field::new(name, dtype.clone()));
        let arr = self.collect_arr_trusted_with_dtype(field.dtype.to_arrow(PlFlavor::highest()));
        ChunkedArray::from_chunk_iter_and_field(field, [arr])
    }
    ...
    {
        let field = Arc::new(Field::new(name, dtype.clone()));
        let arr = self.try_collect_arr_with_dtype(field.dtype.to_arrow(PlFlavor::highest()))?;
        ...
        Self: Iterator<Item = Result<U, E>> + TrustedLen,
    {
        let field = Arc::new(Field::new(name, dtype.clone()));
        let arr =
            self.try_collect_arr_trusted_with_dtype(field.dtype.to_arrow(PlFlavor::highest()))?;
        Ok(ChunkedArray::from_chunk_iter_and_field(field, [arr]))
    }
