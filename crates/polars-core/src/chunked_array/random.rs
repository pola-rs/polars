...
use rand_distr::{Normal, StandardUniform, StandardNormal, Uniform};
    ...
    T::Native: Float,
{
    ...
    pub fn rand_uniform(name: PlSmallStr, length: usize, low: f64, high: f64) -> PolarsResult<Self> {
        let uniform = Uniform::new(low, high).map_err(to_compute_err)?;
        let mut builder = PrimitiveChunkedBuilder::<T>::new(name, length);
        let mut rng = rand::rng();
        for _ in 0..length {
