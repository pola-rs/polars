...
impl<'a, T> ChunkApplyCast<'a, T::Native> for ChunkedArray<T>
where
    T: PolarsNumericType,
{
...
impl<'a> ChunkApplyCast<'a, bool> for BooleanChunked {
    fn apply_cast_numeric<F, S>(&self, f: F) -> ChunkedArray<S>
    where
...
impl<'a> ChunkApplyCast<'a, Series> for ListChunked {
    fn apply_cast_numeric<F, S>(&self, f: F) -> ChunkedArray<S>
    where
...
impl<'a> ChunkApplyCast<'a, &'a str> for Utf8Chunked {
...
impl<'a> ChunkApplyCast<'a, &'a [u8]> for BinaryChunked {
    fn apply_cast_numeric<F, S>(&'a self, f: F) -> ChunkedArray<S>
    where
...
#[cfg(feature = "object")]
impl<'a, T> ChunkApplyCast<'a, &'a T> for ObjectChunked<T>
where
    T: PolarsObject,
{
