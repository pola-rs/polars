...
impl Duration {
    ...
    fn localize_result_rfc_5545(
        ...
        let tz_name = || tz.iana_name().unwrap();
        let out_of_range = || {
            PolarsError::ComputeError(
                format!("datetime '{}' is out-of-range", result_dt_local).into(),
            )
        };
        ...
        match tz.to_ambiguous_timestamp(result_dt_local).offset() {
            ...
            AmbiguousOffset::Fold { before, after } => {
                ...
                polars_bail!(ComputeError: "Could not localize datetime '{}' to time zone '{}'", result_dt_local, tz_name());
            },
...
fn new_datetime(
    ...
    let date = NaiveDate::new(i16::try_from(year).ok()?, month as i8, days as i8).ok()?;
