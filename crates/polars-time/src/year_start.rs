...
pub(crate) fn roll_backward(
    ...
    let date = NaiveDate::from_ymd_opt(ts.year(), 1, 1).ok_or_else(|| {
        polars_err!(
...
pub trait PolarsYearStart {
    fn year_start(&self, time_zone: Option<&Tz>) -> PolarsResult<Self>
    where
        Self: Sized;
}

impl PolarsYearStart for DatetimeChunked {
...
impl PolarsYearStart for DateChunked {
