...
impl TimeMethods for TimeChunked {
    ...
    /// Returns the hour number from 0 to 23.
    fn hour(&self) -> Int8Chunked {
        unary_elementwise(self.physical(), |opt| opt.and_then(time_to_hour))
    }

    /// Extract minute from underlying NaiveDateTime representation.
    /// Returns the minute number from 0 to 59.
    fn minute(&self) -> Int8Chunked {
        unary_elementwise(self.physical(), |opt| opt.and_then(time_to_minute))
    }

    /// Extract second from underlying NaiveDateTime representation.
    /// Returns the second number from 0 to 59.
    fn second(&self) -> Int8Chunked {
        unary_elementwise(self.physical(), |opt| opt.and_then(time_to_second))
    }
    ...
    /// The range from 1,000,000,000 to 1,999,999,999 represents the leap second.
    fn nanosecond(&self) -> Int32Chunked {
        unary_elementwise(self.physical(), |opt| opt.and_then(time_to_nanosecond))
    }
