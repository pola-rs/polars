...
pub trait TemporalMethods: AsSeries {
    ...
    fn minute(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-datetime")]
    ...
    fn second(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-datetime")]
    ...
    fn nanosecond(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-datetime")]
    ...
    fn day(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
    ...
    fn weekday(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-date")]
            DataType::Date => s.date().map(|ca| {
                ...
                unary_elementwise_values(ca.physical(), |t| (((t - 4) % 7 + 7) % 7 + 1) as i64)
            }),
            #[cfg(feature = "dtype-datetime")]
    ...
    fn week(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-date")]
    ...
    fn ordinal_day(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-date")]
    ...
    fn millennium(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            ...
            #[cfg(feature = "dtype-date")]
            DataType::Date => s.date().map(|ca| (ca.year() - 1i64).div(1000f64) + 1),
            #[cfg(feature = "dtype-datetime")]
    ...
    /// Calculate the millennium from the underlying NaiveDateTime representation.
    fn century(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
    ...
    fn year(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-date")]
    ...
    fn iso_year(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-date")]
    ...
    /// Returns the year number in the calendar date.
    fn ordinal_year(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-date")]
            DataType::Date => s.date().map(|ca| ca.year()),
    ...
    fn quarter(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-date")]
    ...
    fn month(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-date")]
    ...
    /// representation.
    fn days_in_month(&self) -> PolarsResult<Int64Chunked> {
        let s = self.as_series();
        match s.dtype() {
            #[cfg(feature = "dtype-date")]
