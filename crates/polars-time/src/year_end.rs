...
use crate::year_start::roll_backward;
...
fn roll_forward(
    ...
    let t = offset_fn(&Duration::parse("12mo"), t, time_zone)?;
    offset_fn(&Duration::parse("-1d"), t, time_zone)
}

pub trait PolarsYearEnd {
    fn year_end(&self, time_zone: Option<&Tz>) -> PolarsResult<Self>
    where
        Self: Sized;
}

impl PolarsYearEnd for DatetimeChunked {
    fn year_end(&self, time_zone: Option<&Tz>) -> PolarsResult<Self> {
...
impl PolarsYearEnd for DateChunked {
