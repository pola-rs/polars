...
#[test]
fn test_select_strict_hconcat_pushdown_cached_27923() -> PolarsResult<()> {
    unsafe {
        std::env::set_var(
    ...
    polars_config::config()
        .reload_env_var("POLARS_PROJECTION_PUSHDOWN_PRUNE_STRICT_HCONCAT_INPUTS");
    ...
    let parent = concat_lf_horizontal([left, right], options)?.cache();
    let lf = concat_lf_horizontal(
        [
            parent.clone().select([col("idx")]),
            parent.select([col("a")]),
        ],
        options,
    )?;
    ...
    let node = plan.lp_top;
    let lp_arena = plan.lp_arena;
    let mut found_projected_left = false;

    assert!(lp_arena.iter(node).all(|(_, plan)| match plan {
        IR::DataFrameScan { schema, .. } if schema.contains("right_unused") => false,
        IR::DataFrameScan {
            schema,
            output_schema,
            ..
        } if schema.contains("idx") => {
            let projection = output_schema.as_ref().unwrap();
            let has_expected_projection =
                projection.len() == 2 && projection.contains("idx") && projection.contains("a");
            found_projected_left |= has_expected_projection;
            has_expected_projection
        },
        _ => true,
    }));
    assert!(found_projected_left);
