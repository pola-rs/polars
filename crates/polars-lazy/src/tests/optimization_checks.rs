...
#[cfg(feature = "merge_sorted")]
fn optimize_lf(lf: LazyFrame) -> PolarsResult<(Node, Arena<IR>)> {
    let (mut expr_arena, mut lp_arena) = get_arenas();
...
#[cfg(feature = "merge_sorted")]
fn assert_balanced_merge_sorted(
    root: Node,
    lp_arena: &Arena<IR>,
    ...
    // Keep the helper iterative; it exercises the same deep trees that motivated
    // the rebalance rule.
    let mut max_depth = 0;
    let mut next_expected = expected_start;
    let mut seen = 0;
    let mut stack = Vec::with_capacity(expected_len.max(1));
    stack.push((root, 0));
...
#[test]
#[cfg(feature = "merge_sorted")]
fn test_rebalance_merge_sorted_plan_shapes() -> PolarsResult<()> {
    for (n_inputs, expected_depth) in [(7_i64, 3_usize), (8, 3)] {
        let (root, lp_arena) = optimize_lf(merge_sorted_chain(n_inputs))?;

        assert_balanced_merge_sorted(root, &lp_arena, expected_depth, 0, n_inputs as usize);
    }
    ...
    let (root, lp_arena) = optimize_lf(concat(
        &[
    ...
    assert_balanced_merge_sorted(inputs[0], &lp_arena, 3, 0, 7);
    assert_balanced_merge_sorted(inputs[1], &lp_arena, 3, 7, 7);

    let (root, lp_arena) = optimize_lf(
        merge_sorted_chain_with_key_and_offset(4, "a", 0)
    ...
    for child in [left, right] {
        match lp_arena.get(child) {
            IR::MergeSorted { key, .. } => assert_eq!(key.as_str(), "a"),
            ir => panic!("expected merge_sorted child, got {ir:?}"),
        }
    }

    assert_balanced_merge_sorted(left, &lp_arena, 2, 0, 4);
    assert_balanced_merge_sorted(right, &lp_arena, 2, 4, 4);
...
fn test_rebalance_merge_sorted_deep_chain_collect_after_sort() -> PolarsResult<()> {
    // Deep enough to trigger the rebalance, but not so deep it needs a siesta.
