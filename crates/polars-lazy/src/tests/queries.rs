...
fn test_lazy_query_2() {
    ...
    let ldf = df
        .lazy()
        .with_column(col("a").map(|s| Ok(s * 2), |_, f| Ok(field.clone())))
...
fn test_lazy_query_4() -> PolarsResult<()> {
    ...
    let out = base_df
        ...
        .agg([
            col("day").alias("day"),
            col("cumcases")
                .apply(|s: Column| (&s - &(s.shift(1))), |_, f| Ok(field.clone()))
                .alias("diff_cases"),
        ])
...
fn test_lazy_group_by_apply() {
    ...
    df.lazy()
        .group_by([col("fruits")])
        .agg([col("cars").apply(
            |s: Column| Ok(Column::new(s.name().clone(), &[s.len() as u32])),
            |_, f| Ok(Field::new(field.name().clone(), DataType::UInt32)),
        )])
        .collect()
        .unwrap();
}
