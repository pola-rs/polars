...
#[test]
#[cfg(feature = "iejoin")]
fn test_join_where_rejects_full() -> PolarsResult<()> {
    let left = df!["a" => [1, 2, 3]]?.lazy();
    let right = df!["b" => [2]]?.lazy();

    let q = left
        .join_builder()
        .with(right)
        .how(JoinType::Full)
        .join_where(vec![col("a").gt(col("b"))]);

    let err = q.collect().unwrap_err().to_string();
    assert!(
        err.contains("join is not supported with non-equi join conditions"),
...
#[test]
#[cfg(feature = "iejoin")]
fn test_join_where_left() -> PolarsResult<()> {
...
#[test]
#[cfg(feature = "iejoin")]
fn test_join_where_right() -> PolarsResult<()> {
    let left = df!["a" => [1, 2, 3]]?.lazy();
    let right = df!["b" => [2]]?.lazy();
