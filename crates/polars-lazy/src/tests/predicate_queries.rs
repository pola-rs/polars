...
#[test]
fn test_right_join_with_cast_predicate_pushdown_26322() -> PolarsResult<()> {
    // The optimizer tries to rewrite right-join to inner-join when it detects
    // a filter that removes NULLs, but fails to unwrap Cast expressions around
    // join keys before checking if they are Column expressions.
    let lhs = df![
        "x" => [0i64, 1i64],
        "z" => [4i64, 5i64],
    ]?
    ...
    // This creates a right join where the join keys have different types (i64 vs i32),
    // causing Cast expressions to be inserted. The filter `z >= 6` removes NULLs
    // from the left side, triggering the optimizer to rewrite right-join to inner-join.
    let q = lhs
        .join(rhs, [col("x")], [col("y")], JoinType::Right.into())
        .filter(col("z").gt_eq(lit(6i64)));

    // This should not panic - it should successfully optimize and return an empty result
    let out = q.collect()?;

    // With z >= 6, no rows from lhs match, so the result should be empty
    assert_eq!(out.height(), 0);
    // Right join output: RHS key column (y) coalesced, then LHS non-key columns (z)
    // Note: Column order depends on the join rewrite (right->inner causes reordering)
