...
**Branch `sql2` @ `d492e8c8d7`, rebased onto main (b72bea9d00), pushed. Conformance: 99.1% (9249 passed / 80 expected-fail / 9329 run; skipif'd records leave the total).**
...
**DONE (03f9e36cdb + d492e8c8d7)**: IN-subquery 3VL — 57-entry in1 cluster closed down to 39 legitimately-out-of-scope leftovers: 28 cascade from the setup shim not supporting `INSERT ... SELECT` (+7 direct), 2 cross-type `is_in` coercion (engine), 2 hex-literal syntax. Remaining baseline (80): those 39 in1 · slt_lang_aggfunc ~13 · engine strays 4 · misc S items · rest small.

**Queue (on resume)**: small strays (JOIN ON shapes 1, DELETE-on-view 1, int-div skipif 2) → optional: `INSERT ... SELECT` support in the harness setup shim (would unlock the 35 in1 cascade entries) → low-corpus-weight items (3b window frames, 3d GROUPING SETS, 3f recursive CTEs). Engine track (separate branch off main): frame-independence-aware `ExprPushdownGroup` so constant subtrees (e.g. `lit(s).implode()`) stop classifying as pushdown barriers.
