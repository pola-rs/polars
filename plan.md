...
**Branch `sql2` @ `7b4eba3a90`, rebased onto main (b72bea9d00), pushed. Conformance: 98.9% (9200 passed / 98 expected-fail / 9298 run; skipif'd records leave the total). PAUSED here by user instruction (2026-07-22): no new agents without go-ahead.**
...
**Queue (on resume)**: in1 NOT-IN-subquery 3VL (57 entries, last big cluster) → small strays (JOIN ON shapes 1, DELETE-on-view 1, int-div skipif 2) → low-corpus-weight items (3b window frames, 3d GROUPING SETS, 3f recursive CTEs). Engine track (separate branch off main): frame-independence-aware `ExprPushdownGroup` so constant subtrees (e.g. `lit(s).implode()`) stop classifying as pushdown barriers.
