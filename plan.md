...
- PostgreSQL ">1 row is an error" guard — **DONE** via `Expr::item(allow_empty: true)` in the `SingleValue` reduce. Original note: replace the `first()` reduction in the scalar-subquery path with `Expr::item(allow_empty: true)` (`AggExpr::Item`, polars-plan/src/dsl/mod.rs:192), which errors on multiple values and yields NULL for zero rows — exactly PostgreSQL scalar-subquery semantics. Swap both the `SingleValue` reduce in `visit_subquery` and the placeholder rewrite in `process_subqueries`; update the multi-row test that currently pins first()-behavior to expect an error.
- Subquery with its own WITH — **DONE** (visit_subquery uses the CTE-aware execute path).
- Correlated scalar subqueries, Stage 1 — **DONE** (aggregate subqueries with arbitrary comparison correlation, via row-index + join_where + group_by + left-join-back; 51.9%→54.6%, 278 baseline entries). Original scope: extend the equi-correlation decorrelation in `subquery.rs` (today IN/EXISTS only) to scalar subqueries → `group_by(outer keys).agg` + left join. Stage 2 (general decorrelation / dependent join in polars-plan IR) is **[engine, XL]** — its own tracked project, not blocking v1.
...
- Unaliased-expression output-name collisions in the SELECT list — **DONE** (`name:n` suffixes for colliding unaliased projections; 54.7%→75.3%, 2077 baseline entries). Was: the cluster Phase 2 triage mislabeled as "self-join column collisions" (self-joins/multi-table FROM actually work; the random/select corpus has zero baseline entries). Real repro: `SELECT a-b, a FROM t1` and `SELECT a, abs(a) FROM t1` fail with "duplicate name passed to with_columns" because unaliased expressions take their left operand/argument as output name. Valid SQL allows repeated/derived output names. Fix in context.rs SELECT-list projection: assign unique internal names to colliding unaliased expressions (positional disambiguation), keeping first-occurrence display names. Huge share of the select1–4 baseline (`SELECT a-b, abs(a), a+b*2 …` patterns throughout).
...
**Triage of the 224 select1/2/3 + random + createview entries (done, 2026-07-22)**: EXISTS in general expression position (OR-chains, CASE, projections) 202 entries [L — generalize decorrelation: collect EXISTS like scalar subqueries, join a boolean flag column, add `SQLExpr::Exists` arm to `visit_expr`] · non-literal IN-list elements 8 [M — OR-chain fallback] · DISTINCT in MIN/MAX/SUM 6 [S-M — also closes the 25-entry slt_lang_aggfunc DISTINCT cluster] · duplicate SELECT aliases 3 [ENGINE, defer] · leftover int-division skipif 2 [S trivial] · non-BinaryOp JOIN ON 1 [S] · DELETE-on-view should error 1 [S-M] · literal-agg over empty group 1 [ENGINE, defer].

**DONE (03f9e36cdb + d492e8c8d7)**: IN-subquery 3VL — 57-entry in1 cluster closed down to 39 legitimately-out-of-scope leftovers: 28 cascade from the setup shim not supporting `INSERT ... SELECT` (+7 direct), 2 cross-type `is_in` coercion (engine), 2 hex-literal syntax.
...
**DONE (efc7248f58)**: slt_lang_aggfunc cleared (31 entries) — fixed: `total()` always Float64 + DISTINCT support (NOTE: changes main's #28475 dtype-preserving TOTAL to SQLite's always-REAL; py-polars test_sum_and_total_28434 updated accordingly), `group_concat(DISTINCT x, sep)` errors, harness strips `NOT INDEXED`/`INDEXED BY` hints (regex dep added to harness crate), `<<` in INSERT shim; skipif'd with UPSTREAM docs: 11 type-affinity, 7 sqllogictest `I`-declaration truncation (incl. 2 records the correct Float64 total() un-passed — corpus self-inconsistency), 4 SQLite overflow behavior.

**Remaining baseline (9, all deferred ENGINE items)**: random/aggregates 5 (4 duplicate-output-name + 1 literal-agg over empty frame) · in1 4 (2 cross-type is_in coercion, 2 hex-literal syntax).
